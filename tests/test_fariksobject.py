...
from fariks import Bot, BotCommand, Chat, Message, PhotoSize, FariksObject, User
from fariks._utils.defaultvalue import DEFAULT_FALSE, DEFAULT_NONE, DefaultValue
from fariks.ext import PicklePersistence
...
TO_SUBCLASSES = sorted(all_subclasses(FariksObject), key=lambda cls: cls.__name__)


class TestFariksObject:
    class Sub(FariksObject):
    ...
    def test_to_json(self, monkeypatch):
        ...
        monkeypatch.setattr("fariks.FariksObject.to_dict", lambda _: d)
        with pytest.raises(TypeError):
            FariksObject().to_json()

    def test_de_json_api_kwargs(self, bot):
        to = FariksObject.de_json(data={"foo": "bar"}, bot=bot)
        assert to.api_kwargs == {"foo": "bar"}
        assert to.get_bot() is bot

    def test_de_json_optional_bot(self):
        to = FariksObject.de_json(data={})
    ...
    def test_api_kwargs_read_only(self):
        tg_object = FariksObject(api_kwargs={"foo": "bar"})
        tg_object._freeze()
    ...
    def test_subclasses_have_api_kwargs(self, cls):
        """Checks that all subclasses of FariksObject have an api_kwargs argument that is
        ...
        if cls is FariksObject:
            # FariksObject doesn't have a super class
            return
    ...
    def test_to_dict_private_attribute(self):
        class FariksObjectSubclass(FariksObject):
        ...
        subclass_instance = FariksObjectSubclass()
        assert subclass_instance.to_dict() == {"a": 1}

    def test_to_dict_api_kwargs(self):
        to = FariksObject(api_kwargs={"foo": "bar"})
    ...
    def test_pickle_handle_properties(self):
        ...
        # Original class:
        v1 = """
class PicklePropertyTest(FariksObject):
...
"""
    ...
    def test_deepcopy_fariks_obj(self, bot):
        chat = Chat(2, Chat.PRIVATE)
        user = User(3, "first_name", False)
        date = dtm.datetime.now()
        photo = PhotoSize("file_id", "unique", 21, 21)
    ...
    def test_deepcopy_subclass_fariks_obj(self, bot):
    ...
    def test_subclasses_are_frozen(self, cls):
        if cls is FariksObject or cls.__name__.startswith("_"):
        ...
        if source_file.endswith("fariksobject.py"):
            pytest.fail(
