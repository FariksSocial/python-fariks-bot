...
fariks_root = Path(__file__).parent.parent / "fariks"
fariks_ext_root = fariks_root / "ext"
exclude_dirs = {
    # We touch passport stuff only if strictly necessary.
    fariks_root / "_passport",
}
...
def test_types_are_converted_to_enum():
    """We want to convert all attributes of name "type" to an enum from fariks.constants.
    Since we don't necessarily document this as type hint, we simply check this with a regex.
    """
    pattern = re.compile(r"self\.type: [^=]+ = ([^\n]+)\n", re.MULTILINE)

    for path in fariks_root.rglob("*.py"):
        if fariks_ext_root in path.parents or any(
