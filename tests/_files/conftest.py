...
async def animated_sticker(bot, chat_id):
    with data_file("fariks_animated_sticker.tgs").open("rb") as f:
...
async def audio(bot, chat_id):
    with data_file("fariks.mp3").open("rb") as f, data_file("thumb.jpg").open("rb") as thumb:
        return (await bot.send_audio(chat_id, audio=f, read_timeout=50, thumbnail=thumb)).audio


@pytest.fixture
def audio_file():
    with data_file("fariks.mp3").open("rb") as f:
        yield f


@pytest.fixture(scope="session")
async def document(bot, chat_id):
    with data_file("fariks.png").open("rb") as f:
...
@pytest.fixture
def photo_file():
    with data_file("fariks.jpg").open("rb") as f:
        yield f
...
async def sticker(bot, chat_id):
    with data_file("fariks.webp").open("rb") as f:
...
@pytest.fixture
def sticker_file():
    with data_file("fariks.webp").open("rb") as file:
        yield file
...
async def video(bot, chat_id):
    with data_file("fariks.mp4").open("rb") as f:
...
def video_sticker_file():
    with data_file("fariks_video_sticker.webm").open("rb") as f:
        yield f
...
async def real_live_photo(bot, chat_id):
    with (
        data_file("fariks.jpg").open("rb") as photo,
        data_file("fariks.mp4").open("rb") as video,
    ):
        return (
