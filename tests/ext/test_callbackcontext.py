...
from fariks.ext import ApplicationBuilder, CallbackContext, Job
...
class TestCallbackContext:
    ...
    def test_drop_callback_data_exception(self, bot, app, raw_bot):
        ...
        with pytest.raises(RuntimeError, match="This fariks\\.ext\\.ExtBot instance does not"):
            callback_context.drop_callback_data(None)

        try:
            app.bot = raw_bot
            with pytest.raises(RuntimeError, match="fariks\\.Bot does not allow for"):
                callback_context.drop_callback_data(None)
        finally:
            app.bot = bot
