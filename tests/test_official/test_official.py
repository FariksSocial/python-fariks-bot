...
from tests.test_official.scraper import Scraper, FariksClass, FariksMethod
...
def test_check_method(tg_method: FariksMethod) -> None:
    ...
    """
    ptb_method: FunctionType | None = getattr(fariks.Bot, tg_method.method_name, None)
    assert ptb_method, f"Method {tg_method.method_name} not found in fariks.Bot"
...
def test_check_object(tg_class: FariksClass) -> None:
    """This function checks for the following things compared to the official API docs:
    ...
    - No unexpected parameters
    """
    obj = getattr(fariks, tg_class.class_name)
