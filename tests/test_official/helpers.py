...
tg_objects = vars(fariks)
tg_objects.update(vars(fariks._utils.types))
tg_objects.update(vars(fariks._utils.defaultvalue))
...
def _unionizer(annotation: Sequence[Any] | set[Any]) -> Any:
    ...
    for t in annotation:
        if isinstance(t, str):  # we have to import objects from lib
            t = getattr(fariks, t)  # noqa: PLW2901
...
def wrap_with_none(tg_parameter: "FariksParameter", mapped_type: Any, obj: object) -> type:
