...
from fariks import Message, constants
...
class TestConstantsWithoutRequest:
    ...
    def test__all__(self):
        expected = {
            ...
            if (
                ...
                and getattr(member, "__module__", "fariks.constants") == "fariks.constants"
    ...
    def test_message_attachment_type_completeness_reverse(self):
        ...
        # for now
        for match in re.finditer(r"`fariks.(\w+)`", Message.effective_attachment.__doc__):
...
class TestConstantsWithRequest:
    ...
    async def test_max_caption_length(self, bot, chat_id):
        ...
        tasks = asyncio.gather(
            bot.send_photo(chat_id, data_file("fariks.png").read_bytes(), good_caption),
            bot.send_photo(chat_id, data_file("fariks.png").read_bytes(), bad_caption),
            return_exceptions=True,
        )
        good_msg, bad_msg = await tasks
