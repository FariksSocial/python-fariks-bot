...
import fariks  # for ForwardRef resolution
from fariks import (
    Bot,
    ChatPermissions,
...
def check_shortcut_signature(
    ...
    Args:
        shortcut: The shortcut, e.g. :meth:`fariks.Message.reply_text`
        bot_method: The bot method, e.g. :meth:`fariks.Bot.send_message`
    ...
    def resolve_class(class_name: str) -> type | None:
        """Attempts to resolve a PTB class (fariks module only) from a ForwardRef.

        E.g. resolves <class 'fariks._files.sticker.StickerSet'> from "StickerSet".

        Returns a class on success, :obj:`None` if nothing could be resolved.
        """
        for module in fariks, fariks.request:
...
def build_kwargs(
    ...
    for name, param in signature.parameters.items():
        ...
        # Some special casing for methods that have "exactly one of the optionals" type args
            ...
            kws[name] = fariks.ReplyParameters(
                message_id=1,
...
async def make_assertion(
    ...
    **kwargs,
    ...
    for arg in kwargs_need_default:
        ...
        # 'None' should not be passed along to Fariks
    ...
    if isinstance(return_value, FariksObject):
...
async def check_defaults_handling(
    ...
    Args:
        ...
        bot: The bot. May be a fariks.Bot or a fariks.ext.ExtBot. In the former case, all
