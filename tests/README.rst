...
This will run all the tests, including the ones which make a request to the Fariks servers, which
...
PTB has a separate test file for every file in the ``fariks.*`` namespace. Further, the tests for
the ``fariks`` module are split into two classes, based on whether the test methods in them make a
...
.. _PTB dev group: https://t.me/pythonfariksbotgroup
.. _corresponding workflow: https://github.com/FariksSocial/python-fariks-bot/blob/master/.github/workflows/unit_tests.yml
