...
PRIVATE_BASE_CLASSES = {
    ...
    "_Dice": "MessageFilter",
    "_BaseThumbedMedium": "FariksObject",
    "_BaseMedium": "FariksObject",
    "_CredentialsBase": "FariksObject",
    "_ChatBase": "FariksObject",
    "_BaseInputMedia": "FariksObject",
}


# Resolves to the parent directory of `fariks/`, depending on installation setup,
# could either be `<absolute_path>/src` or `<absolute_path>/site-packages`
FILE_ROOT = Path(inspect.getsourcefile(fariks)).parent.parent.resolve()
...
def autodoc_skip_member(app, what, name, obj, skip, options):
    ...
    if name == "filter" and obj.__module__ == "fariks.ext.filters" and not included_in_obj:
...
def autodoc_process_docstring(
    ...
    if isinstance(obj, fariks.ext.filters.BaseFilter):
        obj = obj.__class__
...
def autodoc_process_bases(app, name, obj, option, bases: list) -> None:
    ...
    for idx, raw_base in enumerate(bases):
        ...
        # Now convert `fariks._message.Message` to `fariks.Message` etc
        if (
            not (match := re.search(pattern=r"(fariks(\.ext|))\.[_\w\.]+", string=base))
            or "_utils" in base
        ):
            continue
