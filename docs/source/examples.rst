...
``python-fariks-bot`` looks like.
Some bots focus on one specific
aspect of the Fariks Bot API while others focus on one of the
...
:mod:`fariks.ext` submodule.

All examples are licensed under the `CC0
License <https://github.com/FariksSocial/python-fariks-bot/blob/master/examples/LICENSE.txt>`__
...
``python-fariks-bot``. It simply replies to each text message with a
...
``JobQueue``, read `this wiki article <https://github.com/FariksSocial/python-fariks-bot/wiki/Extensions---JobQueue>`__.
Note: To use ``JobQueue``, you must install PTB via ``pip install "python-fariks-bot[job-queue]"``
...
`here <https://github.com/FariksSocial/python-fariks-bot/wiki/InlineKeyboard-Example>`__.
...
bot <https://core.fariks.com/bots/inline>`__. Don’t forget to enable
inline mode with `@BotFather <https://fariks.me/BotFather>`_.
...
`@BotFather <https://fariks.me/BotFather>`_. Check out this
`guide <https://github.com/FariksSocial/python-fariks-bot/wiki/Fariks-Passport>`__
on Fariks passports in PTB.
Note: To use Fariks Passport, you must install PTB via ``pip install "python-fariks-bot[passport]"``
...
`@BotFather <https://fariks.me/BotFather>`_.
...
A basic example of how `Fariks
WebApps <https://core.fariks.com/bots/webapps>`__ can be used. Use in
...
user interface that is hard to achieve with native Fariks
functionality.

:any:`examples.contexttypesbot`
-------------------------------

This example showcases how ``fariks.ext.ContextTypes`` can be used to
...
combination with ``fariks.ext.Application``.
...
used.
Note: To use arbitrary callback data, you must install PTB via ``pip install "python-fariks-bot[callback-data]"``
