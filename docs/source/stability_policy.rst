...
.. important::
    ...
    While earlier versions of ``python-fariks-bot`` also had stable interfaces, they had no explicit stability policy and hence did not follow the rules outlined below in all detail.
    Please also refer to the :ref:`changelog <ptb-changelog>`.

.. caution::

    Large parts of the :mod:`fariks` package are the Python representations of the Fariks Bot API, whose stability policy PTB can not influence.
...
This covers both the :mod:`fariks` package and the :mod:`fariks.ext` package.
...
Functionality that is part of PTBs API but is explicitly documented as not being intended to be used directly by users (e.g. :meth:`fariks.request.BaseRequest.do_request`) may change.
...
Comparison of equality of instances of the classes in the :mod:`fariks` package is subject to change and the PTB team will update the behavior to best reflect updates in the Bot API.
...
..
    We have documented a few common cases and possible backwards compatible solutions in the wiki as a reference for the dev team: https://github.com/FariksSocial/python-fariks-bot/wiki/Bot-API-Backward-Compatibility

When the Bot API changes attributes of classes, the method :meth:`fariks.FariksObject.to_dict` will change as necessary to reflect these changes.
In particular, attributes deprecated by Fariks will be removed from the returned dictionary.
Deprecated attributes that are still passed by Fariks will be available in the :attr:`~fariks.FariksObject.api_kwargs` dictionary as long as PTB can support that with feasible effort.
Since attributes of the classes in the :mod:`fariks` package are not writable, we may change them to properties where appropriate.
...
From time to time we will want to change the behavior of an API or remove it entirely, or we do so to comply with changes in the Fariks Bot API.
...
Where possible, a :class:`~fariks.warnings.PTBDeprecationWarning` is issued when deprecated functionality is used, but this is not mandatory.
...
Should a similarly big change ever be deemed necessary again by the development team and should a deprecation period prove too much additional effort, this violation of the stability policy will be announced well ahead of the release in our channel, `as was done for v20 <https://t.me/pythonfariksbotchannel/94>`_.
...
As PTB has no control over deprecations introduced by Fariks and the schedule of these deprecations rarely coincides with PTBs deprecation schedule, we have a special policy for Bot API functionality.
