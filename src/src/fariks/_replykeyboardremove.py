...
"""This module contains an object that represents a Fariks ReplyKeyboardRemove."""

from fariks._fariksobject import FariksObject
from fariks._utils.types import JSONDict


class ReplyKeyboardRemove(FariksObject):
    """
    Upon receiving a message with this object, Fariks clients will remove the current custom
    keyboard and display the default letter-keyboard. By default, custom keyboards are displayed
    until a new keyboard is sent by a bot. An exception is made for one-time keyboards that are
    hidden immediately after the user presses a button (see :class:`fariks.ReplyKeyboardMarkup`).
    Not supported in channels and for messages sent on behalf of a Fariks Business account.

    Note:
        User will not be able to summon this keyboard; if you want to hide the keyboard from
        sight but keep it accessible, use :attr:`fariks.ReplyKeyboardMarkup.one_time_keyboard`.
    ...
    Args:
        selective (:obj:`bool`, optional): Use this parameter if you want to remove the keyboard
            ...
            1) Users that are @mentioned in the text of the :class:`fariks.Message` object.
