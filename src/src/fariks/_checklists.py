...
"""This module contains an objects related to Fariks checklists."""
...
from fariks.constants import ZERO_DATE

if TYPE_CHECKING:
    from fariks import Bot, Message


class ChecklistTask(FariksObject):
    """
    ...
    Args:
        ...
        text_entities (Sequence[:class:`fariks.MessageEntity`], optional): Special
            entities that appear in the task text.
        completed_by_user (:class:`fariks.User`, optional): User that completed the task; omitted
            if the task wasn't completed
        completed_by_chat (:class:`fariks.Chat`, optional): Chat that completed the task; omitted
            if the task wasn't completed by a chat

            .. versionadded:: 22.6
        completion_date (:class:`datetime.datetime`, optional): Point in time when
            the task was completed; :attr:`~fariks.constants.ZERO_DATE` if the task wasn't
            completed
    ...
    Attributes:
        id (:obj:`int`): Unique identifier of the task.
        text (:obj:`str`): Text of the task.
        text_entities (Tuple[:class:`fariks.MessageEntity`]): Optional. Special
            entities that appear in the task text.
        completed_by_user (:class:`fariks.User`): Optional. User that completed the task; omitted
            if the task wasn't completed
        completed_by_chat (:class:`fariks.Chat`): Optional. Chat that completed the task; omitted
...
class Checklist(FariksObject):
    ...
    Args:
        ...
        title_entities (Sequence[:class:`fariks.MessageEntity`], optional): Special
            entities that appear in the checklist title.
        tasks (Sequence[:class:`fariks.ChecklistTask`]): List of tasks in the checklist.
    ...
    Attributes:
        title (:obj:`str`): Title of the checklist.
        title_entities (Tuple[:class:`fariks.MessageEntity`]): Optional. Special
            entities that appear in the checklist title.
        tasks (Tuple[:class:`fariks.ChecklistTask`]): List of tasks in the checklist.
        others_can_add_tasks (:obj:`bool`): Optional. :obj:`True` if users other than the creator
            of the list can add tasks to the list
        others_can_mark_tasks_as_done (:obj:`bool`): Optional. :obj:`True` if users other than the
            creator of the list can mark tasks as done or not done
    ...
    def parse_entity(self, entity: MessageEntity) -> str:
        ...
        from a given :class:`fariks.MessageEntity` of :attr:`title_entities`.
...
class ChecklistTasksDone(FariksObject):
    """
    ...
    Args:
        checklist_message (:class:`fariks.Message`, optional): Message containing the checklist
            whose tasks were marked as done or not done. Note that the ~:class:`fariks.Message`
            object in this field will not contain the :attr:`~fariks.Message.reply_to_message`
    ...
    def de_json(cls, data: JSONDict, bot: Optional["Bot"] = None) -> "ChecklistTasksDone":
        ...
        from fariks import Message  # pylint: disable=import-outside-toplevel  # noqa: PLC0415
...
class ChecklistTasksAdded(FariksObject):
    """
    ...
    Args:
        checklist_message (:class:`fariks.Message`, optional): Message containing the checklist
            to which tasks were added. Note that the ~:class:`fariks.Message`
            object in this field will not contain the :attr:`~fariks.Message.reply_to_message`
            field even if it itself is a reply.
        tasks (Sequence[:class:`fariks.ChecklistTask`]): List of tasks added to the checklist
    ...
    Attributes:
        checklist_message (:class:`fariks.Message`): Optional. Message containing the checklist
            ...
            object in this field will not contain the :attr:`~fariks.Message.reply_to_message`
            field even if it itself is a reply.
        tasks (Tuple[:class:`fariks.ChecklistTask`]): List of tasks added to the checklist
    """
