...
def get_logger(file_name: str, class_name: str | None = None) -> logging.Logger:
    ...
    If for example `__name__` is `fariks.ext._updater`, the logger will be named
    `fariks.ext.Updater`. If `class_name` is passed, this will result in
    `fariks.ext.<class_name>`. Useful e.g. for CamelCase class names.

    If the file name points to a utils module, the logger name will simply be `fariks(.ext)`.
