...
from fariks._utils.types import HTTPVersion, ODVInput, SocketOpt
from fariks.error import NetworkError, TimedOut
from fariks.request._baserequest import BaseRequest
...
class HTTPXRequest(BaseRequest):
    """Implementation of :class:`~fariks.request.BaseRequest` using the library
    ...
    Args:
        ...
        read_timeout (:obj:`float` | :obj:`None`, optional): If passed, specifies the maximum
            amount of time (in seconds) to wait for a response from Fariks's server.
        ...
        pool_timeout (:obj:`float` | :obj:`None`, optional): If passed, specifies the maximum
            ...
            Warning:
                With a finite pool timeout, you must expect :exc:`fariks.error.TimedOut`
        ...
        proxy (:obj:`str` | ``httpx.Proxy`` | ``httpx.URL``, optional): The URL to a proxy server,
            ...
            Note:
                ...
                * For Socks5 support, additional dependencies are required. Make sure to install
                  PTB via :command:`pip install "python-fariks-bot[socks]"` in this case.
        ...
        httpx_kwargs (dict[:obj:`str`, Any], optional): Additional keyword arguments to be passed
            ...
            Warning:
                ...
                defaults set by ``python-fariks-bot`` and all other parameters passed to
    ...
    async def do_request(
        ...
        # The Fariks server expects a typed application/json body for non-file
        # requests (form-encoded bodies are rejected with HTTP 415). Use
        # multipart only when uploading files; otherwise send the parameters as
        # a JSON object with their native types (numbers stay numbers, etc.).
        if files:
            ...
            data = None
            content = (
                json.dumps(request_data.parameters).encode("utf-8")
                if request_data
                else None
            )
            request_headers = {
            ...
            if isinstance(err, httpx.PoolTimeout):
                raise TimedOut(
                    message=(
                        ...
                        "Request was *not* sent to Fariks. Consider adjusting the connection "
