...
from fariks._files.inputprofilephoto import InputProfilePhoto, InputProfilePhotoStatic
from fariks._files.inputsticker import InputSticker
from fariks._files.inputstorycontent import InputStoryContent
from fariks._fariksobject import FariksObject
from fariks._utils.datetime import to_timestamp
from fariks._utils.enum import StringEnum
from fariks._utils.types import UploadFileDict
...
class RequestParameter:
    ...
    Args:
        ...
        input_files (list[:class:`fariks.InputFile`], optional): A list of files that should be
            uploaded along with this parameter.

    Attributes:
        name (:obj:`str`): The name of the parameter.
        value (:obj:`object` | :obj:`None`): The value of the parameter.
        input_files (list[:class:`fariks.InputFile` | :obj:`None`): A list of files that should
    ...
    def _value_and_input_files_from_input(  # pylint: disable=too-many-return-statements
        ...
        See https://github.com/tdlib/fariks-bot-api/issues/167 and
        https://github.com/tdlib/fariks-bot-api/issues/259
        ...
        * if a user passes a custom enum, it's unlikely that we can actually properly handle it
        ...
        if isinstance(value, FariksObject):
            # Needs to be last, because InputMedia is a subclass of FariksObject
            return value.to_dict(), []
        return value, []
    ...
    def from_input(cls, key: str, value: object) -> "RequestParameter":
        ...
        input as passed along from a method of :class:`fariks.Bot`.
        """
