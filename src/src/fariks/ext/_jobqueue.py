...
from fariks._utils.datetime import UTC, localize
...
from fariks.ext._utils.types import CCT, JobCallback
...
class JobQueue(Generic[CCT]):
    ...
    async def job_callback(job_queue: "JobQueue[CCT]", job: "Job[CCT]") -> None:
        ...
        containing the :class:`JobQueue` itself and the :class:`~fariks.ext.Job` instance.
        ...
        Hint:
            This method is effectively a wrapper for :meth:`fariks.ext.Job.run`.

        .. versionadded:: 20.4

        Args:
            job_queue (:class:`JobQueue`): The job queue that created the job.
            job (:class:`~fariks.ext.Job`): The job to run.
        """
    ...
    def run_once(
        ...
        Args:
            ...
            when (:obj:`int` | :obj:`float` | :obj:`datetime.timedelta` |                         \
                ...
                * :obj:`datetime.datetime` will be interpreted as a specific date and time at
                  ...
                  :attr:`fariks.ext.Defaults.tzinfo` is used.
            ...
            chat_id (:obj:`int`, optional): Chat id of the chat associated with this job. If
                passed, the corresponding :attr:`~fariks.ext.CallbackContext.chat_data` will
                be available in the callback.

                .. versionadded:: 20.0

            user_id (:obj:`int`, optional): User id of the user associated with this job. If
                passed, the corresponding :attr:`~fariks.ext.CallbackContext.user_data` will
                be available in the callback.
        ...
        Returns:
            :class:`fariks.ext.Job`: The new :class:`Job` instance that has been added to the job
            queue.
    ...
    def run_repeating(
        ...
        Args:
            ...
            first (:obj:`int` | :obj:`float` | :obj:`datetime.timedelta` |                        \
                ...
                * :obj:`datetime.time` will be interpreted as a specific time of day at which the
                ...
                Note:
                    ...
                    .. seealso:: :meth:`fariks.ext.Job.run`
            ...
            last (:obj:`int` | :obj:`float` | :obj:`datetime.timedelta` |                        \
                ...
                assumed, which is UTC unless :attr:`fariks.ext.Defaults.tzinfo` is used.
    ...
    def run_monthly(
        ...
        Args:
            ...
            when (:obj:`datetime.time`): Time of day at which the job should run. If the timezone
                ...
                which is UTC unless :attr:`fariks.ext.Defaults.tzinfo` is used.
    ...
    def run_daily(
        ...
        Args:
            ...
            time (:obj:`datetime.time`): Time of day at which the job should run. If the timezone
                ...
                be used, which is UTC unless :attr:`fariks.ext.Defaults.tzinfo` is used.
    ...
    async def start(self) -> None:
        ...
        """Starts the :class:`~fariks.ext.JobQueue`."""
        if not self.scheduler.running:
            self.scheduler.start()

    async def stop(self, wait: bool = True) -> None:
        """Shuts down the :class:`~fariks.ext.JobQueue`.
...
class Job(Generic[CCT]):
    """This class is a convenience wrapper for the jobs held in a :class:`fariks.ext.JobQueue`.
    ...
    Note:
        ...
        attributes/methods of the corresponding :class:`fariks.ext.Job` object.

    Warning:
        This class should not be instantiated manually.
        Use the methods of :class:`fariks.ext.JobQueue` to schedule jobs.
    ...
    def __getattr__(self, item: str) -> object:
        ...
        :class:`fariks.ext.Job` object or of its attribute :class:`apscheduler.job.Job`,
        if exists.
        ...
        Raises:
            :exc:`AttributeError`: If the attribute does not exist in both
                :class:`fariks.ext.Job` and :class:`apscheduler.job.Job` objects.
        """
        try:
            return getattr(self.job, item)
        except AttributeError as exc:
            raise AttributeError(
                f"Neither 'fariks.ext.Job' nor 'apscheduler.job.Job' has attribute '{item}'"
            ) from exc

    def __eq__(self, other: object) -> bool:
        """Defines equality condition for the :class:`fariks.ext.Job` object.
    ...
    def next_t(self) -> dtm.datetime | None:
        ...
        Warning:
            This attribute is only available, if the :class:`fariks.ext.JobQueue` this job
    ...
    def from_aps_job(cls, aps_job: "APSJob") -> "Job[CCT]":
        """Provides the :class:`fariks.ext.Job` that is associated with the given APScheduler
        job.

        Tip:
            This method can be useful when using advanced APScheduler features along with
            :class:`fariks.ext.JobQueue`.
    ...
    async def run(
        ...
        :meth:`fariks.ext.Application.update_persistence`.

        .. versionchanged:: 20.0
            Calls :meth:`fariks.ext.Application.update_persistence`.

        Args:
            application (:class:`fariks.ext.Application`): The application this job is associated
                with.
        """
