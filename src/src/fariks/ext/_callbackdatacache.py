...
from fariks import CallbackQuery, InlineKeyboardMarkup, Message, User
from fariks._utils.datetime import to_float_timestamp
from fariks.error import FariksError
from fariks.ext._utils.types import CDCData

if TYPE_CHECKING:
    from fariks.ext import ExtBot


class InvalidCallbackData(FariksError):
    """
...
class CallbackDataCache:
    """A custom cache for storing the callback data of a :class:`fariks.ext.ExtBot`. Internally,
    ...
    * One for mapping the IDs of received callback queries to the cached objects
    ...
    Args:
        bot (:class:`fariks.ext.ExtBot`): The bot this cache is for.
        ...
        :meth:`fariks.ext.BasePersistence.get_callback_data`.
    ...
    def __init__(
        ...
        if not CACHE_TOOLS_AVAILABLE:
            raise RuntimeError(
                ...
                '"python-fariks-bot[callback-data]"`.'
            )
    ...
    def process_keyboard(self, reply_markup: InlineKeyboardMarkup) -> InlineKeyboardMarkup:
        ...
        :attr:`~fariks.InlineKeyboardButton.callback_data`, stores that data and builds a new
        keyboard with the correspondingly replaced buttons. Otherwise, does nothing and returns
        the original reply markup.

        Args:
            reply_markup (:class:`fariks.InlineKeyboardMarkup`): The keyboard.

        Returns:
            :class:`fariks.InlineKeyboardMarkup`: The keyboard to be passed to Fariks.
    ...
    def extract_uuids(callback_data: str) -> tuple[str, str]:
        ...
        Args:
            callback_data (:obj:`str`): The
                :paramref:`~fariks.InlineKeyboardButton.callback_data` as present in the button.
    ...
    def process_message(self, message: Message) -> None:
        ...
        :class:`fariks.ext.InvalidCallbackData` will be inserted.

        Note:
            Checks :attr:`fariks.Message.via_bot` and :attr:`fariks.Message.from_user` to check
        ...
        Warning:
            * Does *not* consider :attr:`fariks.Message.reply_to_message` and
              :attr:`fariks.Message.pinned_message`. Pass them to this method separately.
            * *In place*, i.e. the passed :class:`fariks.Message` will be changed!

        Args:
            message (:class:`fariks.Message`): The message.
    ...
    def process_callback_query(self, callback_query: CallbackQuery) -> None:
        ...
        :class:`fariks.ext.InvalidCallbackData` will be inserted.
        If :attr:`fariks.CallbackQuery.data` or :attr:`fariks.CallbackQuery.message` is
        ...
        Note:
            ...
            :attr:`fariks.Message.reply_to_message` and :attr:`fariks.Message.pinned_message`
            if necessary.

        Warning:
            *In place*, i.e. the passed :class:`fariks.CallbackQuery` will be changed!
