...
class CommandHandler(BaseHandler[Update, CCT, RT]):
    """Handler class to handle Fariks commands.

    Commands are Fariks messages that start with a :attr:`fariks.MessageEntity.BOT_COMMAND`
    ...
    use :attr:`~filters.UpdateType.EDITED_MESSAGE <fariks.ext.filters.UpdateType.EDITED_MESSAGE>`
    in the filter argument.

    Note:
        :class:`CommandHandler` does *not* handle (edited) channel posts and does *not* handle
        commands that are part of a caption. Please use :class:`~fariks.ext.MessageHandler`
        with a suitable combination of filters (e.g.
        :attr:`fariks.ext.filters.UpdateType.CHANNEL_POSTS`,
        :attr:`fariks.ext.filters.CAPTION` and :class:`fariks.ext.filters.Regex`) to handle
        those messages.

    Note:
        If you want to support a different entity in the beginning, e.g. if a command message is
        wrapped in a :attr:`fariks.MessageEntity.CODE`, use the
        :class:`fariks.ext.PrefixHandler`.
    ...
    Args:
        command (:obj:`str` | Collection[:obj:`str`]):
            ...
            Limitations are the same as for :attr:`fariks.BotCommand.command`.
        ...
        filters (:class:`fariks.ext.filters.BaseFilter`, optional): A filter inheriting from
    ...
    Attributes:
        ...
        filters (:class:`fariks.ext.filters.BaseFilter`): Optional. Only allow updates with these
            filters.
