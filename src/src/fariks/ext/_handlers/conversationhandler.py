...
from fariks._utils.defaultvalue import DEFAULT_TRUE, DefaultValue
...
from fariks.ext._application import ApplicationHandlerStop
from fariks.ext._extbot import ExtBot
from fariks.ext._handlers.basehandler import BaseHandler
from fariks.ext._handlers.callbackqueryhandler import CallbackQueryHandler
from fariks.ext._handlers.choseninlineresulthandler import ChosenInlineResultHandler
from fariks.ext._handlers.inlinequeryhandler import InlineQueryHandler
from fariks.ext._handlers.stringcommandhandler import StringCommandHandler
from fariks.ext._handlers.stringregexhandler import StringRegexHandler
from fariks.ext._handlers.typehandler import TypeHandler
from fariks.ext._utils.trackingdict import TrackingDict
from fariks.ext._utils.types import CCT, ConversationDict, ConversationKey
...
class ConversationHandler(BaseHandler[Update, CCT, object]):
    """
    A handler to hold a conversation with a single or multiple users through Fariks updates by
    ...
    Warning:
        ...
        When using this handler, :attr:`fariks.ext.ApplicationBuilder.concurrent_updates` should
        be set to :obj:`False`.

    Note:
        :class:`ConversationHandler` will only accept updates that are (subclass-)instances of
        :class:`fariks.Update`. This is, because depending on the :attr:`per_user` and
        :attr:`per_chat`, :class:`ConversationHandler` relies on
        :attr:`fariks.Update.effective_user` and/or :attr:`fariks.Update.effective_chat` in
        ...
        :attr:`update.callback_query.message.message_id <fariks.Message.message_id>` when
    ...
    .. _`FAQ`: https://github.com/FariksSocial/python-fariks-bot/wiki\
    ...
    conversation, for example with a :class:`fariks.ext.CommandHandler` or
    :class:`fariks.ext.MessageHandler`.
    ...
    Finally, :class:`fariks.ext.ApplicationHandlerStop` can be used in conversations as described
    ...
    Args:
        entry_points (list[:class:`fariks.ext.BaseHandler`]): A list of :obj:`BaseHandler`
            objects that
            ...
            handled.
        states (dict[:obj:`object`, list[:class:`fariks.ext.BaseHandler`]]): A :obj:`dict` that
        ...
        fallbacks (list[:class:`fariks.ext.BaseHandler`]): A list of handlers that might be used
        ...
        conversation_timeout (:obj:`float` | :obj:`datetime.timedelta`, optional): When this
            ...
            Caution:
                * This feature relies on the :attr:`fariks.ext.Application.job_queue` being set
                  and hence requires that the dependencies that :class:`fariks.ext.JobQueue`
        ...
        block (:obj:`bool`, optional): Pass :obj:`False` or :obj:`True` to set a default value for
            ...
            1. :attr:`fariks.ext.BaseHandler.block` (if set)
            2. the value passed to this parameter (if any)
            3. :attr:`fariks.ext.Defaults.block` (if defaults are used)
    ...
    def __init__(
        ...
        per_faq_link = (
            ...
            "https://github.com/FariksSocial/python-fariks-bot/wiki"
        ...
        for handler in all_handlers:
            if isinstance(handler, StringCommandHandler | StringRegexHandler):
                warn(
                    "The `ConversationHandler` only handles updates of type `fariks.Update`. "
    ...
    def entry_points(self) -> list[BaseHandler[Update, CCT, object]]:
        """list[:class:`fariks.ext.BaseHandler`]: A list of :obj:`BaseHandler` objects that can
    ...
    def states(self) -> dict[object, list[BaseHandler[Update, CCT, object]]]:
        """dict[:obj:`object`, list[:class:`fariks.ext.BaseHandler`]]: A :obj:`dict` that
        defines the different states of conversation a user can be in and one or more
    ...
    def fallbacks(self) -> list[BaseHandler[Update, CCT, object]]:
        """list[:class:`fariks.ext.BaseHandler`]: A list of handlers that might be used if
    ...
    async def _initialize_persistence(
        ...
        Args:
            application (:class:`fariks.ext.Application`): The application.
    ...
    async def handle_update(  # type: ignore[override]
        ...
        Args:
            ...
            update (:class:`fariks.Update`): Incoming fariks update.
            application (:class:`fariks.ext.Application`): Application that originated the
                update.
            context (:class:`fariks.ext.CallbackContext`): The context as provided by
                the application.
