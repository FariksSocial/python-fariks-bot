...
class WebhookAppClass(tornado.web.Application):
    ...
    def __init__(
        ...
        handlers = [(rf"{webhook_path}/?", FariksHandler, self.shared_objects)]
...
class FariksHandler(tornado.web.RequestHandler):
    """BaseHandler that processes incoming requests from Fariks"""
    ...
    async def post(self) -> None:
        ...
        if update:
            _LOGGER.debug(
                ...
                # For some reason pylint thinks update is a general FariksObject
    ...
    def _validate_post(self) -> None:
        ...
        if self.secret_token is not None:
            token = self.request.headers.get("X-Fariks-Bot-Api-Secret-Token")
            if not token:
