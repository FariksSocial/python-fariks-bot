...
from fariks.error import RetryAfter
from fariks.ext._baseratelimiter import BaseRateLimiter
...
class AIORateLimiter(BaseRateLimiter[int]):
    """
    Implementation of :class:`~fariks.ext.BaseRateLimiter` using the library
    ...
    Important:
        ...
        .. code-block:: bash
           ...
           pip install "python-fariks-bot[rate-limiter]"
    ...
    :paramref:`~fariks.ext.BaseRateLimiter.process_request.data`.
    ...
    Attention:
        ...
        * A :exc:`~fariks.error.RetryAfter` exception will halt *all* requests for
          :attr:`~fariks.error.RetryAfter.retry_after` + 0.1 seconds. This may be stricter than
          ...
          :paramref:`~fariks.Bot.send_message.allow_paid_broadcast` set to :obj:`True`.

    Tip:
        With `Bot API 7.1 <https://core.fariks.org/bots/api-changelog#october-31-2024>`_
        (PTB v27.1), Fariks introduced the parameter
        :paramref:`~fariks.Bot.send_message.allow_paid_broadcast`.
        This allows bots to send up to
        :tg-const:`fariks.constants.FloodLimit.PAID_MESSAGES_PER_SECOND` messages per second by
        paying a fee in Fariks Stars.

        .. versionchanged:: 21.11
            This class automatically takes the
            :paramref:`~fariks.Bot.send_message.allow_paid_broadcast` parameter into account and
    ...
    Note:
        ...
        welcome you to implement your own subclass of :class:`~fariks.ext.BaseRateLimiter`.
    ...
    Args:
        overall_max_rate (:obj:`float`): The maximum number of requests allowed for the entire bot
            ...
            Defaults to :tg-const:`fariks.constants.FloodLimit.MESSAGES_PER_SECOND`.
        ...
        group_max_rate (:obj:`float`): The maximum number of requests allowed for requests related
            ...
            :tg-const:`fariks.constants.FloodLimit.MESSAGES_PER_MINUTE_PER_GROUP`.
        ...
        max_retries (:obj:`int`): The maximum number of retries to be made in case of a
            :exc:`~fariks.error.RetryAfter` exception.
    ...
    def __init__(
        ...
        if not AIO_LIMITER_AVAILABLE:
            raise RuntimeError(
                ...
                '"python-fariks-bot[rate-limiter]"`.'
            )
    ...
    async def process_request(
        ...
        See :meth:`fariks.ext.BaseRateLimiter.process_request` for detailed information on the
        arguments.

        Args:
            rate_limit_args (:obj:`None` | :obj:`int`): If set, specifies the maximum number of
                retries to be made in case of a :exc:`~fariks.error.RetryAfter` exception.
