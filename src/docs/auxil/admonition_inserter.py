...
import fariks
import fariks._utils.defaultvalue
import fariks._utils.types
import fariks.ext
import fariks.ext._utils.types
...
TG_NAMESPACE = vars(fariks).copy()
TG_NAMESPACE.update(vars(fariks._utils.types))
TG_NAMESPACE.update(vars(fariks._utils.defaultvalue))
TG_NAMESPACE.update(vars(fariks.ext))
TG_NAMESPACE.update(vars(fariks.ext._utils.types))
TG_NAMESPACE.update(vars(fariks.ext._applicationbuilder))
...
class AdmonitionInserter:
    """Class for inserting admonitions into docs of Fariks classes."""
    ...
    METHOD_NAMES_FOR_BOT_APP_APPBUILDER: typing.ClassVar[dict[type, str]] = {
        ...
        for cls in (fariks.Bot, fariks.ext.ApplicationBuilder, fariks.ext.Application)
    }
    ...
    Methods must be public, not aliases, not inherited from FariksObject.
    """
    ...
    def __init__(self):
        ...
        Each sub-dictionary matches bot methods (for "Shortcuts") or fariks classes (for other
        admonition types) to texts of admonitions, e.g.:

        ```
        {
            "use_in": {
                <class 'fariks._chatinvitelink.ChatInviteLink'>:
    ...
    def _create_available_in(self) -> dict[type, str]:
        ...
        # i.e. {fariks._files.sticker.Sticker: {":attr:`fariks.Message.sticker`", ...}}
        attrs_for_class = defaultdict(set)

        classes_to_inspect = inspect.getmembers(fariks, inspect.isclass) + inspect.getmembers(
            fariks.ext, inspect.isclass
        )

        for _class_name, inspected_class in classes_to_inspect:
            # We need to make "<class 'fariks._files.sticker.StickerSet'>" into
            # "fariks.StickerSet" because that's the way the classes are mentioned in
            # docstrings.
    ...
    def _create_returned_in(self) -> dict[type, str]:
        ...
        # i.e. {<class 'fariks._message.Message'>: {:meth:`fariks.Bot.send_message`, ...}}
    ...
    def _create_shortcuts(self) -> dict[collections.abc.Callable, str]:
        ...
        # for, i.e. {<function Bot.send_voice at ...>: {:meth:`fariks.User.send_voice`, ...}
        shortcuts_for_bot_method = defaultdict(set)

        # inspect methods of all fariks classes for return statements that indicate
        # that this given method is a shortcut for a Bot method
        for _class_name, cls in inspect.getmembers(fariks, predicate=inspect.isclass):
            if not cls.__module__.startswith("fariks"):
                # For some reason inspect.getmembers() also yields some classes that are
                # imported in the namespace but not part of the fariks module.
                continue

            if cls is fariks.Bot:
            ...
            for method_name, method in _iter_own_public_methods(cls):
                ...
                for line in inspect.getsourcelines(method)[0]:
                    ...
                    bot_method = getattr(fariks.Bot, bot_method_match.group())
    ...
    def _create_use_in(self) -> dict[type, str]:
        ...
        # i.e. {<class 'fariks._inline.inlinequeryresult.InlineQueryResult'>:
    ...
    def _generate_admonitions(
        ...
        ```
        {<class 'fariks._files.sticker.StickerSet'>:
        ...
        for cls, attrs in attrs_or_methods_for_class.items():
            if cls is fariks.ext.ApplicationBuilder:
    ...
    def _resolve_arg_and_add_link(
        ...
        **Modifies dictionary in place.**
        ...
        for cls in self._resolve_arg(type_hints, resolve_nested_type_vars):
            ...
            # Also, if class was resolved correctly, "fariks" will definitely be in its str().
            if cls is None or "fariks" not in str(cls):
                continue
    ...
    def _resolve_arg(
        ...
        def _is_ptb_class(cls: type) -> bool:
            if not hasattr(cls, "__module__"):
                return False
            return cls.__module__.startswith("fariks")
        ...
        def recurse_type(type_, is_recursed_from_ptb_class: bool):
            ...
            if hasattr(type_, "__origin__") or isinstance(
                ...
                # Make sure it's not a fariks.ext generic type (e.g. ContextTypes[...])
                org = typing.get_origin(type_)
                if "fariks.ext" in str(org):
                    fariks_classes.add(org)
            ...
            elif inspect.isclass(type_) and "fariks" in inspect.getmodule(type_).__name__:
                fariks_classes.add(type_)
                ...
                with contextlib.suppress(AttributeError):
                    fariks_classes.add(self._resolve_class(type_.__forward_arg__))

        for type_hint in type_hints.values():
            if type_hint is not None:
                recurse_type(type_hint, False)

        return list(fariks_classes)

    @staticmethod
    def _resolve_class(name: str) -> type | None:
        """The keys in the admonitions dictionary are not strings like "fariks.StickerSet"
        but classes like <class 'fariks._files.sticker.StickerSet'>.

        This method attempts to resolve a PTB class from a name that does or does not
        contain the word 'fariks', e.g.
        <class 'fariks._files.sticker.StickerSet'> from "fariks.StickerSet" or "StickerSet".
        ...
        for option in (
            ...
            f"fariks.{name}",
            f"fariks.ext.{name}",
            f"fariks.ext.filters.{name}",
        ):
            # NameError will be raised if trying to eval just name and it doesn't work, e.g.
            # "Name 'ApplicationBuilder' is not defined".
            # AttributeError will be raised if trying to e.g. eval f"fariks.{name}" when the
            # class denoted by `name` actually belongs to `fariks.ext`:
            # "module 'fariks' has no attribute 'ApplicationBuilder'".
