...
class TGConstXRefRole(PyXRefRole):
    ...
    reference values from the `fariks.constants.module` while using the actual value as title
    of the link.

    Example:

        :tg-const:`fariks.constants.MessageLimit.MAX_TEXT_LENGTH` renders as `4096` but links to
        the constant.
    """
    ...
    def process_link(
        ...
        try:
            ...
            if isinstance(value, Enum):
                ...
                if isinstance(value, fariks.constants.FileSizeLimit):
            ...
            if isinstance(value, str) and target in (
                "fariks.constants.BOT_API_VERSION",
                "fariks.__version__",
            ):
                return value, target
            if isinstance(value, tuple) and target in (
                "fariks.constants.BOT_API_VERSION_INFO",
                "fariks.__version_info__",
            ):
                return str(value), target
            if (
                isinstance(value, dtm.datetime)
                and value == fariks.constants.ZERO_DATE
                and target == "fariks.constants.ZERO_DATE"
            ):
