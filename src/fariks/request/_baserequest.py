...
from fariks._utils.defaultvalue import DEFAULT_NONE as _DEFAULT_NONE
...
from fariks._version import __version__ as ptb_ver
from fariks.error import (
    BadRequest,
    ChatMigrated,
    Conflict,
    Forbidden,
...
from fariks.request._requestdata import RequestData
...
class BaseRequest(
    ...
    """Abstract interface class that allows python-fariks-bot to make requests to the Bot API.
    ...
    Tip:
        ...
        custom logic to encode the keys of :attr:`fariks.request.RequestData.parameters`.
    ...
    USER_AGENT: Final[str] = f"python-fariks-bot v{ptb_ver} (https://python-fariks-bot.org)"
    ...
    async def post(
        ...
        Warning:
            This method will be called by the methods of :class:`fariks.Bot` and should *not* be
            called manually.

        Args:
            url (:obj:`str`): The URL to request.
            request_data (:class:`fariks.request.RequestData`, optional): An object containing
                information about parameters and files to upload for the request.
            read_timeout (:obj:`float` | :obj:`None`, optional): If passed, specifies the maximum
                amount of time (in seconds) to wait for a response from Fariks's server instead
        ...
        # see https://core.fariks.org/bots/api#making-requests
    ...
    async def _request_wrapper(
        ...
        * Parse the Fariks server response.
        ...
        parsing_exception: FariksError | None = None

        try:
            response_data = self.parse_json_payload(payload)
        except FariksError as exc:
            ...
            # see https://core.fariks.org/bots/api#responseparameters and
            # https://core.fariks.org/bots/api#making-requests
        ...
        if code == HTTPStatus.FORBIDDEN:  # 403
            exception: FariksError = Forbidden(message)
    ...
    def parse_json_payload(payload: bytes) -> JSONDict:
        """Parse the JSON returned from Fariks.
        ...
        Args:
            payload (:obj:`bytes`): The UTF-8 encoded JSON payload as returned by Fariks.

        Returns:
            dict: A JSON parsed as Python dict with results.

        Raises:
            FariksError: If loading the JSON data failed
        """
        ...
        try:
            ...
            raise FariksError("Invalid server response") from exc
