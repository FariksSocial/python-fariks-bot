...
from fariks._utils.types import FileInput, FilePathInput

if TYPE_CHECKING:
    from typing import Any

    from fariks import InputFile, FariksObject
...
def parse_file_input(  # pylint: disable=too-many-return-statements
    ...
    * For IO and bytes input, returns an :class:`fariks.InputFile`.
    * If :attr:`tg_type` is specified and the input is of that type, returns the ``file_id``
      attribute.

    Args:
        file_input (:obj:`str` | :obj:`bytes` | :term:`file object` | :class:`~fariks.InputFile`\
            | Fariks media object): The input to parse.
        tg_type (:obj:`type`, optional): The Fariks media type the input can be. E.g.
            :class:`fariks.Animation`.
        filename (:obj:`str`, optional): The filename. Only relevant in case an
            :class:`fariks.InputFile` is returned.
        attach (:obj:`bool`, optional): Pass :obj:`True` if the parameter this file belongs to in
            the request to Fariks should point to the multipart data via an ``attach://`` URI.
            Defaults to `False`. Only relevant if an :class:`fariks.InputFile` is returned.
        local_mode (:obj:`bool`, optional): Pass :obj:`True` if the bot is running an api server
            in ``--local`` mode.

    Returns:
        :obj:`str` | :class:`fariks.InputFile` | :obj:`object`: The parsed input or the untouched
        :attr:`file_input`, in case it's no valid file input.
    """
    # Importing on file-level yields cyclic Import Errors
    from fariks import InputFile  # pylint: disable=import-outside-toplevel  # noqa: PLC0415
