...
from fariks.ext._utils.types import BD, BT, CD, UD

if TYPE_CHECKING:
    from asyncio import Future, Queue

    from fariks.ext import Application, Job, JobQueue
    from fariks.ext._utils.types import CCT

_STORING_DATA_WIKI = (
    "https://github.com/FariksSocial/python-fariks-bot"
...
class CallbackContext(Generic[BT, UD, CD, BD]):
    """
    This is a context object passed to the callback called by :class:`fariks.ext.BaseHandler`
    or by the :class:`fariks.ext.Application` in an error handler added by
    :attr:`fariks.ext.Application.add_error_handler` or to the callback of a
    :class:`fariks.ext.Job`.

    Note:
        :class:`fariks.ext.Application` will create a single context for an entire update. This
    ...
    Warning:
         Do not combine custom attributes with :paramref:`fariks.ext.BaseHandler.block` set to
         :obj:`False` or :attr:`fariks.ext.Application.concurrent_updates` set to
    ...
    .. seealso:: :attr:`fariks.ext.ContextTypes.DEFAULT_TYPE`,
        :wiki:`Job Queue <Extensions---JobQueue>`

    Args:
        application (:class:`fariks.ext.Application`): The application associated with this
            context.
    ...
    Attributes:
        ...
        args (list[:obj:`str`]): Optional. Arguments passed to a command if the associated update
            is handled by :class:`fariks.ext.CommandHandler`, :class:`fariks.ext.PrefixHandler`
            or :class:`fariks.ext.StringCommandHandler`. It contains a list of the words in the
            text after the command, using any whitespace string as a delimiter.
        error (:exc:`Exception`): Optional. The error that was raised. Only present when passed
            to an error handler registered with :attr:`fariks.ext.Application.add_error_handler`.
        job (:class:`fariks.ext.Job`): Optional. The job which originated this callback.
            Only present when passed to the callback of :class:`fariks.ext.Job` or in error
    ...
    def application(self) -> "Application[BT, ST, UD, CD, BD, Any]":
        """:class:`fariks.ext.Application`: The application associated with this context."""
    ...
    async def refresh_data(self) -> None:
        ...
        :meth:`fariks.ext.BasePersistence.refresh_bot_data` on :attr:`bot_data`,
        :meth:`fariks.ext.BasePersistence.refresh_chat_data` on :attr:`chat_data` and
        :meth:`fariks.ext.BasePersistence.refresh_user_data` on :attr:`user_data`, if
        appropriate.

        Will be called by :meth:`fariks.ext.Application.process_update` and
        :meth:`fariks.ext.Job.run`.
    ...
    def drop_callback_data(self, callback_query: CallbackQuery) -> None:
        ...
        Args:
            callback_query (:class:`fariks.CallbackQuery`): The callback query.
        ...
        if isinstance(self.bot, ExtBot):
            if self.bot.callback_data_cache is None:
                raise RuntimeError(
                    "This fariks.ext.ExtBot instance does not use arbitrary callback data."
                )
            self.bot.callback_data_cache.drop_data(callback_query)
        else:
            raise RuntimeError(  # noqa: TRY004
                "fariks.Bot does not allow for arbitrary callback data."
            )
    ...
    def from_error(
        ...
        """
        Constructs an instance of :class:`fariks.ext.CallbackContext` to be passed to the error
        handlers.

        .. seealso:: :meth:`fariks.ext.Application.add_error_handler`

        .. versionchanged:: 20.0
            Removed arguments ``async_args`` and ``async_kwargs``.

        Args:
            update (:obj:`object` | :class:`fariks.Update`): The update associated with the
                error. May be :obj:`None`, e.g. for errors in job callbacks.
            error (:obj:`Exception`): The error.
            application (:class:`fariks.ext.Application`): The application associated with this
                context.
            job (:class:`fariks.ext.Job`, optional): The job associated with the error.
        ...
        Returns:
            :class:`fariks.ext.CallbackContext`
        """
    ...
    def from_update(
        ...
        """
        Constructs an instance of :class:`fariks.ext.CallbackContext` to be passed to the
        handlers.

        .. seealso:: :meth:`fariks.ext.Application.add_handler`

        Args:
            update (:obj:`object` | :class:`fariks.Update`): The update.
            application (:class:`fariks.ext.Application`): The application associated with this
                context.
    ...
    def from_job(
        ...
        """
        Constructs an instance of :class:`fariks.ext.CallbackContext` to be passed to a
        job callback.

        .. seealso:: :meth:`fariks.ext.JobQueue`

        Args:
            job (:class:`fariks.ext.Job`): The job.
            application (:class:`fariks.ext.Application`): The application associated with this
                context.
    ...
    @property
    def bot(self) -> BT:
        """:class:`fariks.Bot`: The bot associated with this context."""
    ...
    def update_queue(self) -> "Queue[object]":
        ...
        :class:`asyncio.Queue`: The :class:`asyncio.Queue` instance used by the
            :class:`fariks.ext.Application` and (usually) the :class:`fariks.ext.Updater`
