...
from fariks._utils.defaultvalue import (
    DEFAULT_80,
    DEFAULT_IP,
    DEFAULT_NONE,
    DEFAULT_TRUE,
...
from fariks._utils.types import SCT, DVType, ODVInput, TimePeriod
from fariks._utils.warnings import warn
from fariks.error import FariksError
from fariks.ext._basepersistence import BasePersistence
...
from fariks.ext._utils.networkloop import network_retry_loop
from fariks.ext._utils.stack import was_called_by
from fariks.ext._utils.trackingdict import TrackingDict
from fariks.ext._utils.types import BD, BT, CCT, CD, JQ, RT, UD, ConversationKey, HandlerCallback
from fariks.warnings import PTBDeprecationWarning

if TYPE_CHECKING:
    from socket import socket

    from fariks import Message
    from fariks.ext import ConversationHandler, JobQueue
    from fariks.ext._applicationbuilder import InitApplicationBuilder
    from fariks.ext._baseupdateprocessor import BaseUpdateProcessor
    from fariks.ext._jobqueue import Job
...
class ApplicationHandlerStop(Exception):
    ...
    In order to use this exception in a :class:`fariks.ext.ConversationHandler`, pass the
...
class Application(
    ...
    Tip:
         This class may not be initialized directly. Use :class:`fariks.ext.ApplicationBuilder`
    ...
    1. The type of :attr:`bot`. Must be :class:`fariks.Bot` or a subclass of that class.
    2. The type of the argument ``context`` of callback functions for (error) handlers and jobs.
       Must be :class:`fariks.ext.CallbackContext` or a subclass of that class. This must be
    ...
    6. The type of :attr:`job_queue`. Must either be :class:`fariks.ext.JobQueue` or a subclass
       of that or :obj:`None`.
    ...
    .. versionchanged:: 20.0
        ...
        * Initialization is now done through the :class:`fariks.ext.ApplicationBuilder`.
        * Removed the attribute ``groups``.

    Attributes:
        bot (:class:`fariks.Bot`): The bot object that should be passed to the handlers.
        update_queue (:class:`asyncio.Queue`): The synchronized queue that will contain the
            updates.
        updater (:class:`fariks.ext.Updater`): Optional. The updater used by this application.
        chat_data (:obj:`types.MappingProxyType`): A dictionary handlers can use to store data for
            ...
            available as :attr:`fariks.ext.CallbackContext.chat_data` in handler callbacks for
        ...
        user_data (:obj:`types.MappingProxyType`): A dictionary handlers can use to store data for
            ...
            available as :attr:`fariks.ext.CallbackContext.user_data` in handler callbacks for
        ...
        persistence (:class:`fariks.ext.BasePersistence`): The persistence class to
            store data that should be persistent over restarts.
        handlers (dict[:obj:`int`, list[:class:`fariks.ext.BaseHandler`]]): A dictionary mapping
        ...
        context_types (:class:`fariks.ext.ContextTypes`): Specifies the types used by this
    ...
    def __init__(
        ...
        *,
        ...
        if persistence and not isinstance(persistence, BasePersistence):
            raise TypeError("persistence must be based on fariks.ext.BasePersistence")
    ...
    def concurrent_updates(self) -> int:
        ...
        .. versionchanged:: 20.4
            ...
            <fariks.ext.BaseUpdateProcessor.max_concurrent_updates>`.
    ...
    def job_queue(self) -> "JobQueue[CCT] | None":
        """
        :class:`fariks.ext.JobQueue`: The :class:`JobQueue` used by the
        ...
        if self._job_queue is None:
            warn(
                ...
                '`pip install "python-fariks-bot[job-queue]"`.',
                stacklevel=2,
            )
    ...
    def update_processor(self) -> "BaseUpdateProcessor":
        """:class:`fariks.ext.BaseUpdateProcessor`: The update processor used by this
        application.
    ...
    def builder() -> "InitApplicationBuilder":
        """Convenience method. Returns a new :class:`fariks.ext.ApplicationBuilder`.

        .. versionadded:: 20.0
        """
        # Unfortunately this needs to be here due to cyclical imports
        from fariks.ext import (  # pylint: disable=import-outside-toplevel  # noqa: PLC0415
            ApplicationBuilder,
        )
    ...
    async def initialize(self) -> None:
        ...
        * The :attr:`bot`, by calling :meth:`fariks.Bot.initialize`.
        * The :attr:`updater`, by calling :meth:`fariks.ext.Updater.initialize`.
        * The :attr:`persistence`, by loading persistent conversations and data.
        * The :attr:`update_processor` by calling
          :meth:`fariks.ext.BaseUpdateProcessor.initialize`.
        ...
        from fariks.ext._handlers.conversationhandler import ConversationHandler  # noqa: PLC0415
    ...
    async def shutdown(self) -> None:
        ...
        * :attr:`bot` by calling :meth:`fariks.Bot.shutdown`
        * :attr:`updater` by calling :meth:`fariks.ext.Updater.shutdown`
        * :attr:`persistence` by calling :meth:`update_persistence` and
          :meth:`BasePersistence.flush`
        * :attr:`update_processor` by calling :meth:`fariks.ext.BaseUpdateProcessor.shutdown`
    ...
    async def start(self) -> None:
        ...
        * a background task that calls :meth:`update_persistence` in regular intervals, if
        ...
        Note:
            This does *not* start fetching updates from Fariks. To fetch updates, you need to
    ...
    async def stop(self) -> None:
        ...
        Note:
            * This does *not* stop :attr:`updater`. You need to either manually call
              :meth:`fariks.ext.Updater.stop` or use one of :meth:`run_polling` or
              :meth:`run_webhook`.
    ...
    def run_polling(
        ...
        polling updates from Fariks using :meth:`fariks.ext.Updater.start_polling` and
        ...
        - :meth:`fariks.ext.Updater.stop`
        - :meth:`stop`
        - :meth:`post_stop`
        - :meth:`shutdown`
        - :meth:`post_shutdown`

        A small wrapper is passed to :paramref:`fariks.ext.Updater.start_polling.error_callback`
        ...
        .. versionchanged::
            ...
            :class:`fariks.ext.ApplicationBuilder` instead.

        Args:
            poll_interval (:obj:`float`, optional): Time to wait between polling updates from
                Fariks in seconds. Default is ``0.0``.
            timeout (:obj:`int` | :class:`datetime.timedelta`, optional): Passed to
                :paramref:`fariks.Bot.get_updates.timeout`.
            ...
            bootstrap_retries (:obj:`int`, optional): Whether the bootstrapping phase
                ...
                :meth:`fariks.ext.Updater.start_polling`)
                will retry on failures on the Fariks server.
            ...
            drop_pending_updates (:obj:`bool`, optional): Whether to clean any pending updates on
                Fariks servers before actually starting to poll. Default is :obj:`False`.
        ...
        Raises:
            :exc:`RuntimeError`: If the Application does not have an :class:`fariks.ext.Updater`.
        """
        if not self.updater:
            raise RuntimeError(
        ...
        def error_callback(exc: FariksError) -> None:
    ...
    def run_webhook(
        ...
        """Convenience method that takes care of initializing and starting the app,
        listening for updates from Fariks using :meth:`fariks.ext.Updater.start_webhook` and
        a graceful shutdown of the app on exit.
        ...
        ``https://listen:port/url_path``. Also calls :meth:`fariks.Bot.set_webhook` as
        required.
        ...
        Important:
            ...
            .. code-block:: bash
               ...
               pip install "python-fariks-bot[webhooks]"
        ...
        Args:
            ...
            port (:obj:`int`, optional): Port the bot should be listening on. Must be one of
                :attr:`fariks.constants.SUPPORTED_WEBHOOK_PORTS` unless the bot is running
            ...
            drop_pending_updates (:obj:`bool`, optional): Whether to clean any pending updates on
                Fariks servers before actually starting to poll. Default is :obj:`False`.
            ip_address (:obj:`str`, optional): Passed to :meth:`fariks.Bot.set_webhook`.
            max_connections (:obj:`int`, optional): Passed to
                :meth:`fariks.Bot.set_webhook`. Defaults to ``40``.
            close_loop (:obj:`bool`, optional): If :obj:`True`, the current event loop will be
                closed upon shutdown. Defaults to :obj:`True`.
            ...
            secret_token (:obj:`str`, optional): Secret token to ensure webhook requests originate
                from Fariks. See :paramref:`fariks.Bot.set_webhook.secret_token` for more
                details.

                When added, the web server started by this call will expect the token to be set in
                the ``X-Fariks-Bot-Api-Secret-Token`` header of an incoming request and will
    ...
    async def process_update(self, update: object) -> None:
        ...
        .. versionchanged:: 20.0
            ...
            :attr:`fariks.ext.BasePersistence.update_interval`.

        Args:
            update (:class:`fariks.Update` | :obj:`object` | \
                :class:`fariks.error.FariksError`): The update to process.
    ...
    def add_handler(self, handler: BaseHandler[Any, CCT, Any], group: int = DEFAULT_GROUP) -> None:
        ...
        update with :class:`fariks.ext.ApplicationHandlerStop`.

        A handler must be an instance of a subclass of :class:`fariks.ext.BaseHandler`. All
        handlers
        are organized in groups with a numeric value. The default group is 0. All groups will be
        evaluated for handling an update, but only 0 or 1 handler per group will be used. If
        :class:`fariks.ext.ApplicationHandlerStop` is raised from one of the handlers, no further
        ...
        * The first handler in a group which can handle an update (see
          :attr:`fariks.ext.BaseHandler.check_update`) will be used. Other handlers from the
          group will not be used. The order in which handlers were added to the group defines the
          priority.

        Warning:
            Adding persistent :class:`fariks.ext.ConversationHandler` after the application has
        ...
        Args:
            handler (:class:`fariks.ext.BaseHandler`): A BaseHandler instance.
    ...
    def add_handlers(
        ...
        Args:
            handlers (Sequence[:class:`fariks.ext.BaseHandler`] | \
                dict[int, Sequence[:class:`fariks.ext.BaseHandler`]]):
    ...
    def remove_handler(
        ...
        Args:
            handler (:class:`fariks.ext.BaseHandler`): A :class:`fariks.ext.BaseHandler`
                instance.
    ...
    def drop_chat_data(self, chat_id: int) -> None:
        ...
        Warning:
            ...
            :meth:`process_update` or :meth:`fariks.ext.Job.run` may re-create this entry due to
    ...
    def migrate_chat_data(
        ...
        Warning:
            ...
            * This does not update the :attr:`~fariks.ext.Job.chat_id` attribute of any scheduled
              :class:`fariks.ext.Job`.

            When using :attr:`concurrent_updates` or the :attr:`job_queue`,
            :meth:`process_update` or :meth:`fariks.ext.Job.run` may re-create the old entry due
        ...
        Args:
            message (:class:`fariks.Message`, optional): A message with either
                :attr:`~fariks.Message.migrate_from_chat_id` or
                :attr:`~fariks.Message.migrate_to_chat_id`.
                Mutually exclusive with passing :paramref:`old_chat_id` and
                :paramref:`new_chat_id`.

                .. seealso::
                    :attr:`fariks.ext.filters.StatusUpdate.MIGRATE`
    ...
    async def update_persistence(self) -> None:
        ...
        along with :attr:`~fariks.ext.ExtBot.callback_data_cache` and the conversation states of
        any persistent :class:`~fariks.ext.ConversationHandler` registered for this application.
        ...
        .. seealso:: :attr:`fariks.ext.BasePersistence.update_interval`,
    ...
    async def __update_persistence(self) -> None:
        ...
        # Unfortunately due to circular imports this has to be here
        # pylint: disable=import-outside-toplevel
        from fariks.ext._handlers.conversationhandler import PendingState  # noqa: PLC0415
    ...
    def add_error_handler(
        ...
        Args:
            callback (:term:`coroutine function`): The callback function for this error handler.
                ...
                :attr:`fariks.ext.CallbackContext.error`.
    ...
    async def process_error(
        ...
        :class:`fariks.ext.ApplicationHandlerStop`, the error will not be handled by other error
        handlers. Raising :class:`fariks.ext.ApplicationHandlerStop` also stops processing of
        ...
        .. versionchanged:: 20.0
            ...
            * :class:`fariks.ext.ApplicationHandlerStop` is no longer reraised but converted into
              the return value.

        Args:
            update (:obj:`object` | :class:`fariks.Update`): The update that caused the error.
            error (:obj:`Exception`): The error that was raised.
            job (:class:`fariks.ext.Job`, optional): The job that caused the error.
        ...
        Returns:
            ...
            :class:`fariks.ext.ApplicationHandlerStop`. :obj:`False`, otherwise.
        """
