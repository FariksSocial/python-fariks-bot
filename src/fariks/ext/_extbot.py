...
"""This module contains an object that represents a Fariks Bot with convenience extensions."""
...
from fariks._utils.datetime import to_timestamp
...
from fariks.ext._callbackdatacache import CallbackDataCache
from fariks.ext._utils.types import RLARGS
from fariks.request import BaseRequest
from fariks.warnings import PTBUserWarning
...
if TYPE_CHECKING:
    ...
    from fariks.ext import BaseRateLimiter, Defaults
...
class ExtBot(Bot, Generic[RLARGS]):
    """This object represents a Fariks Bot with convenience extensions.

    Warning:
        Not to be confused with :class:`fariks.Bot`.
    ...
    :paramref:`fariks.ext.BaseRateLimiter.process_request.rate_limit_args`.
    ...
    Warning:
        * The keyword argument ``rate_limit_args`` can `not` be used, if :attr:`rate_limiter`
          is :obj:`None`.
        * The method :meth:`~fariks.Bot.get_updates` is the only method that does not have the
    ...
    .. versionchanged:: 20.0
        ...
        :attr:`bot.callback_data_cache.maxsize <fariks.ext.CallbackDataCache.maxsize>` to
    ...
    Args:
        defaults (:class:`fariks.ext.Defaults`, optional): An object containing default values to
            be used if not set explicitly in the bot methods.
        arbitrary_callback_data (:obj:`bool` | :obj:`int`, optional): Whether to
            allow arbitrary objects as callback data for :class:`fariks.InlineKeyboardButton`.
            Pass an integer to specify the maximum number of objects cached in memory.
            Defaults to :obj:`False`.

            .. seealso:: :wiki:`Arbitrary callback_data <Arbitrary-callback_data>`
        rate_limiter (:class:`fariks.ext.BaseRateLimiter`, optional): A rate limiter to use for
    ...
    def callback_data_cache(self) -> CallbackDataCache | None:
        """:class:`fariks.ext.CallbackDataCache`: Optional. The cache for
        objects passed as callback data for :class:`fariks.InlineKeyboardButton`.
        ...
        .. versionchanged:: 20.0
           ...
           * This property is now optional and can be :obj:`None` if
             :paramref:`~fariks.ext.ExtBot.arbitrary_callback_data` is set to :obj:`False`.
        """
        return self._callback_data_cache

    async def initialize(self) -> None:
        """See :meth:`fariks.Bot.initialize`. Also initializes the
        :paramref:`ExtBot.rate_limiter` (if set)
        by calling :meth:`fariks.ext.BaseRateLimiter.initialize`.
        """
    ...
    async def shutdown(self) -> None:
        """See :meth:`fariks.Bot.shutdown`. Also shuts down the
        :paramref:`ExtBot.rate_limiter` (if set) by
        calling :meth:`fariks.ext.BaseRateLimiter.shutdown`.
        """
    ...
    def _merge_api_rl_kwargs(
        ...
        that we can extract them later without having to modify the `fariks.Bot` class.
        """
    ...
    def insert_callback_data(self, update: Update) -> None:
        ...
        Note:
            Checks :attr:`fariks.Message.via_bot` and :attr:`fariks.Message.from_user`
            ...
            Note that this will fail for channel posts, as :attr:`fariks.Message.from_user` is
            :obj:`None` for those! In the corresponding reply markups, the callback data will be
            replaced by :class:`fariks.ext.InvalidCallbackData`.

        Warning:
            *In place*, i.e. the passed :class:`fariks.Message` will be changed!

        Args:
            update (:class:`fariks.Update`): The update.
