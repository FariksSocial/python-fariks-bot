...
"""This module contains the class Updater, which tries to make creating Fariks bots intuitive."""
...
class Updater(contextlib.AbstractAsyncContextManager["Updater"]):
    ...
    .. versionchanged:: 20.0
        ...
        * The only arguments and attributes are now :attr:`bot` and :attr:`update_queue` as now
          ...
          is now :class:`fariks.ext.Application`.

    Args:
        bot (:class:`fariks.Bot`): The bot used with this Updater.
    ...
    async def shutdown(self) -> None:
        """
        Shutdown the Updater & the associated :attr:`bot` by calling :meth:`fariks.Bot.shutdown`.
    ...
    async def start_polling(
        ...
        error_callback: Callable[[FariksError], None] | None = None,
    ) -> "asyncio.Queue[object]":
        """Starts polling updates from Fariks.
        ...
        .. versionchanged:: 22.0
            ...
            the corresponding methods of :class:`fariks.ext.ApplicationBuilder`. or
            by specifying the timeout via :paramref:`fariks.Bot.get_updates_request`.

        Args:
            poll_interval (:obj:`float`, optional): Time to wait between polling updates from
                Fariks in seconds. Default is ``0.0``.
            timeout (:obj:`int` | :class:`datetime.timedelta`, optional): Passed to
                :paramref:`fariks.Bot.get_updates.timeout`. Defaults to
            ...
            drop_pending_updates (:obj:`bool`, optional): Whether to clean any pending updates on
                ...
                .. versionadded :: 13.4
            error_callback (Callable[[:exc:`fariks.error.FariksError`], :obj:`None`], \
                optional): Callback to handle :exc:`fariks.error.FariksError` s that occur
                while calling :meth:`fariks.Bot.get_updates` during polling. Defaults to
                :obj:`None`, in which case errors will be logged. Callback signature::

                    def callback(error: fariks.error.FariksError)
        ...
        async with self.__lock:
            ...
            try:
                ...
                _LOGGER.debug("Polling updates from Fariks started")
            except Exception:
                self._running = False
                raise
    ...
    async def _start_polling(
        ...
        ready: asyncio.Event,
        error_callback: Callable[[FariksError], None] | None,
    ) -> None:
        ...
        async def polling_action_cb() -> None:
            try:
                ...
                # FariksErrors should be processed by the network retry loop
                raise
            except Exception as exc:
                # Other exceptions should not. Let's log them for now.
                _LOGGER.critical(
                    "Something went wrong processing the data received from Fariks. "
    ...
    async def start_webhook(
        ...
        ``https://listen:port/url_path``. Also calls :meth:`fariks.Bot.set_webhook` as required.
        ...
        .. versionchanged:: 13.4
            :meth:`start_webhook` now *always* calls :meth:`fariks.Bot.set_webhook`, so pass
        ...
        Args:
            ...
            ip_address (:obj:`str`, optional): Passed to :meth:`fariks.Bot.set_webhook`.
                ...
                .. versionadded :: 13.4
            allowed_updates (Sequence[:obj:`str`], optional): Passed to
                :meth:`fariks.Bot.set_webhook`. Defaults to :obj:`None`.
            ...
            max_connections (:obj:`int`, optional): Passed to
                ...
                .. versionadded:: 13.6
            secret_token (:obj:`str`, optional): Passed to :meth:`fariks.Bot.set_webhook`.
                Defaults to :obj:`None`.
    ...
    async def _start_webhook(
        ...
        if cert is not None and key is not None:
            try:
                ...
                raise FariksError("Invalid SSL Certificate") from exc
        else:
            ssl_ctx = None
    ...
    async def _bootstrap(
        ...
        async def bootstrap_del_webhook() -> None:
            ...
            if drop_pending_updates:
                _LOGGER.debug("Dropping pending updates from Fariks server")
