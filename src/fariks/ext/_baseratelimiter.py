...
class BaseRateLimiter(ABC, Generic[RLARGS]):
    """
    Abstract interface class that allows to rate limit the requests that python-fariks-bot
    sends to the Fariks Bot API. An implementation of this class
    ...
    :meth:`process_request` and the methods of :class:`~fariks.ext.ExtBot`.

    Hint:
        Requests to :meth:`~fariks.Bot.get_updates` are never rate limited.
    ...
    async def process_request(
        ...
        If a :exc:`~fariks.error.RetryAfter` error is raised, this method may try to make
        ...
        1. React only if necessary. In this case, the :paramref:`callback` is called without any
           precautions. If a :exc:`~fariks.error.RetryAfter` error is raised, processing requests
           is halted for the :attr:`~fariks.error.RetryAfter.retry_after` and finally the
        ...
        Examples:
            * It is usually desirable to call :meth:`fariks.Bot.answer_inline_query`
              as quickly as possible, while delaying :meth:`fariks.Bot.send_message`
              is acceptable.
            * There are `different <https://core.fariks.org/bots/faq\
        ...
        Args:
            ...
            data (dict[:obj:`str`, :obj:`object`]): The parameters that were passed to the method
                of :class:`~fariks.ext.ExtBot`. Any ``api_kwargs`` are included in this and
                any :paramref:`~fariks.ext.ExtBot.defaults` are already applied.
            ...
            rate_limit_args (:obj:`None` | :class:`object`): Custom arguments passed to the methods
                of :class:`~fariks.ext.ExtBot`. Can e.g. be used to specify the priority of
                the request.
