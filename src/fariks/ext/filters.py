...
"""
This module contains filters for use with :class:`fariks.ext.MessageHandler`,
:class:`fariks.ext.CommandHandler`, or :class:`fariks.ext.PrefixHandler`.
...
from fariks import Chat as TGChat
from fariks import (
    Message,
    MessageEntity,
...
from fariks import User as TGUser
from fariks._utils.types import SCT
from fariks.constants import DiceEmoji as DiceEmojiEnum
from fariks.ext._utils._update_parsing import parse_chat_id, parse_username
from fariks.ext._utils.types import FilterDataDict
...
class BaseFilter:
    ...
    Args:
        ...
        data_filter (:obj:`bool`): Whether this filter is a data filter. A data filter should
            ...
            :class:`fariks.ext.CallbackContext`'s internal dict in most cases
    ...
    def check_update(self, update: Update) -> bool | FilterDataDict | None:
        ...
        .. versionchanged:: 21.1
            ...
            :attr:`~fariks.Update.business_message`
            or :attr:`~fariks.Update.edited_business_message`.

        .. versionchanged:: 22.8
            This filter now also returns :obj:`True` if the update contains
            :attr:`~fariks.Update.guest_message`.

        Args:
            update (:class:`fariks.Update`): The update to check.

        Returns:
            :obj:`bool`: :obj:`True` if the update contains one of
            :attr:`~fariks.Update.channel_post`, :attr:`~fariks.Update.message`,
            :attr:`~fariks.Update.edited_channel_post`,
            :attr:`~fariks.Update.edited_message`, :attr:`fariks.Update.business_message`,
            :attr:`fariks.Update.edited_business_message`,
            :attr:`fariks.Update.guest_message`, or :obj:`False` otherwise.
        """
...
class MessageFilter(BaseFilter):
    ...
    to :meth:`filter` is :attr:`fariks.Update.effective_message`.
    ...
    def check_update(self, update: Update) -> bool | FilterDataDict | None:
        ...
        :attr:`~fariks.Update.effective_message` to :meth:`filter`.
    ...
    def filter(self, message: Message) -> bool | FilterDataDict | None:
        ...
        Args:
            message (:class:`fariks.Message`): The message that is tested.
...
class UpdateFilter(BaseFilter):
    ...
    passed to :meth:`filter` is an instance of :class:`fariks.Update`, which allows to create
    filters like :attr:`fariks.ext.filters.UpdateType.EDITED_MESSAGE`.

    Please see :class:`fariks.ext.filters.BaseFilter` for details on how to create custom
    filters.
    ...
    def filter(self, update: Update) -> bool | FilterDataDict | None:
        ...
        Args:
            update (:class:`fariks.Update`): The update that is tested.
...
"""Messages that contain :attr:`fariks.Message.animation`."""
...
"""Messages that contain :meth:`fariks.Message.effective_attachment`.
...
"""Messages that contain :attr:`fariks.Message.audio`."""
...
class Caption(MessageFilter):
    ...
    .. seealso::
        :attr:`fariks.ext.filters.CAPTION`
...
CAPTION = Caption()
"""Shortcut for :class:`fariks.ext.filters.Caption()`.
...
class CaptionEntity(MessageFilter):
    """
    Filters media messages to only allow those which have a :class:`fariks.MessageEntity`
    where their :class:`~fariks.MessageEntity.type` matches `entity_type`.
    ...
    Args:
        entity_type (:obj:`str`): Caption Entity type to check for. All types can be found as
            constants in :class:`fariks.MessageEntity`.
...
"""Messages that contain :attr:`fariks.Message.checklist`.
...
class Command(MessageFilter):
    """
    Messages with a :attr:`fariks.MessageEntity.BOT_COMMAND`. By default, only allows
    ...
    .. seealso::
        :attr:`fariks.ext.filters.COMMAND`.

    Note:
        :attr:`fariks.ext.filters.TEXT` also accepts messages containing a command.
...
COMMAND = Command()
"""Shortcut for :class:`fariks.ext.filters.Command()`.
...
"""Messages that contain :attr:`fariks.Message.contact`."""
...
.. seealso:: :attr:`fariks.Chat.is_direct_messages`
...
class Document:
    ...
    """Messages that contain a :attr:`fariks.Message.document`."""
...
"""Messages that contain :attr:`fariks.Message.effect_id`.
...
class Entity(MessageFilter):
    """
    Filters messages to only allow those which have a :class:`fariks.MessageEntity`
    where their :class:`~fariks.MessageEntity.type` matches `entity_type`.
...
"""Messages that contain :attr:`fariks.Message.forward_origin`.

.. versionchanged:: 20.8
   Now based on :attr:`fariks.Message.forward_origin` instead of
   ``fariks.Message.forward_date``.
"""
...
class ForwardedFrom(_ChatUserBaseFilter):
    ...
    or username(s) based on :attr:`fariks.Message.forward_origin` and in particular

    * :attr:`fariks.MessageOriginUser.sender_user`
    * :attr:`fariks.MessageOriginChat.sender_chat`
    * :attr:`fariks.MessageOriginChannel.chat`

    .. versionadded:: 13.5

    .. versionchanged:: 20.8
       Was previously based on ``fariks.Message.forward_from`` and
         ``fariks.Message.forward_from_chat``.
    ...
    Note:
        ...
        :attr:`fariks.Message.forward_origin` will be of type
        :class:`fariks.MessageOriginHiddenUser`. However, this behaviour
        is undocumented and might be changed by Fariks.
...
"""Messages that contain :attr:`fariks.Message.game`."""
...
"""Messages that contain :attr:`fariks.Message.giveaway`."""
...
"""Messages that contain :attr:`fariks.Message.giveaway_winners`."""
...
"""Messages that contain :attr:`fariks.Message.has_media_spoiler`.
...
"""Messages that contain :attr:`fariks.Message.has_protected_content`.
...
"""Messages that contain :attr:`fariks.Message.invoice`."""
...
"""Messages that contain :attr:`fariks.Message.is_automatic_forward`.
...
"""Messages that contain :attr:`fariks.Message.is_topic_message`.
...
"""Messages that contain :attr:`fariks.Message.is_from_offline`.
...
class Language(MessageFilter):
    ...
    Note:
        According to official Fariks Bot API documentation, not every single user has the
...
"""Messages that contain :attr:`fariks.Message.live_photo`.
...
"""Messages that contain :attr:`fariks.Message.location`."""
...
class Mention(MessageFilter):
    ...
    Args:
        mentions (:obj:`int` | :obj:`str` | :class:`fariks.User` | Collection[:obj:`int` | \
            :obj:`str` | :class:`fariks.User`]):
...
"""Messages that contain :attr:`fariks.Message.paid_media`.
...
"""Messages that contain :attr:`fariks.Message.passport_data`."""
...
"""Messages that contain :attr:`fariks.Message.photo`."""
...
"""Messages that contain :attr:`fariks.Message.poll`."""
...
class Regex(MessageFilter):
    ...
    To get the groups and groupdict matched, see :attr:`fariks.ext.CallbackContext.matches`.
    ...
    Note:
        ...
        With a :attr:`fariks.Message.text` of `x`, will only ever return the matches for the
...
"""Messages that contain :attr:`fariks.Message.reply_to_message`."""
...
class SenderChat(_ChatUserBaseFilter):
    ...
    Note:
        ...
        group). Since v13.9, the field :attr:`fariks.Message.is_automatic_forward` will be
        :obj:`True` for the discussion group message.

    .. seealso:: :attr:`fariks.ext.filters.IS_AUTOMATIC_FORWARD`
    ...
    """All messages with a :attr:`fariks.Message.sender_chat`."""
...
class StatusUpdate:
    ...
    """Messages that contain :attr:`fariks.Message.chat_background_set`."""
    ...
    """Messages that contain :attr:`fariks.Message.group_chat_created`,
        :attr:`fariks.Message.supergroup_chat_created` or
        :attr:`fariks.Message.channel_chat_created`."""
    ...
    """Messages that contain :attr:`fariks.Message.chat_owner_changed`.
    ...
    """Messages that contain :attr:`fariks.Message.chat_owner_left`.
    ...
    """Messages that contain :attr:`fariks.Message.chat_shared`.
    ...
    """Messages that contain :attr:`fariks.Message.checklist_tasks_added`.
    ...
    """Messages that contain :attr:`fariks.Message.checklist_tasks_done`.
    ...
    """Messages that contain :attr:`fariks.Message.connected_website`."""
    ...
    """Messages that contain :attr:`fariks.Message.direct_message_price_changed`.
    ...
    """Messages that contain :attr:`fariks.Message.delete_chat_photo`."""
    ...
    """Messages that contain :attr:`fariks.Message.forum_topic_closed`.
    ...
    """Messages that contain :attr:`fariks.Message.forum_topic_created`.
    ...
    """Messages that contain :attr:`fariks.Message.forum_topic_edited`.
    ...
    """Messages that contain :attr:`fariks.Message.forum_topic_reopened`.
    ...
    """Messages that contain :attr:`fariks.Message.general_forum_topic_hidden`.
    ...
    """Messages that contain :attr:`fariks.Message.general_forum_topic_unhidden`.
    ...
    """Messages that contain :attr:`fariks.Message.gift`.
    ...
    """Messages that contain :attr:`fariks.Message.gift_upgrade_sent`.
    ...
    """Messages that contain :attr:`fariks.Message.giveaway_created`.
    ...
    """Messages that contain :attr:`fariks.Message.giveaway_completed`.
    .. versionadded:: 20.8
    """
    ...
    """Messages that contain :attr:`fariks.Message.left_chat_member`."""
    ...
    """Messages that contain :attr:`fariks.Message.managed_bot_created`."""
    ...
    """Messages that contain :attr:`fariks.Message.message_auto_delete_timer_changed`
    ...
    """Messages that contain :attr:`fariks.Message.migrate_from_chat_id` or
        :attr:`fariks.Message.migrate_to_chat_id`."""
    ...
    """Messages that contain :attr:`fariks.Message.new_chat_members`."""
    ...
    """Messages that contain :attr:`fariks.Message.new_chat_photo`."""
    ...
    """Messages that contain :attr:`fariks.Message.new_chat_title`."""
    ...
    """Messages that contain :attr:`fariks.Message.paid_message_price_changed`.
    ...
    """Messages that contain :attr:`fariks.Message.pinned_message`."""
    ...
    """Messages that contain :attr:`fariks.Message.poll_option_added`.
    .. versionadded:: 22.8
    """
    ...
    """Messages that contain :attr:`fariks.Message.poll_option_deleted`.
    .. versionadded:: 22.8
    """
    ...
    """Messages that contain :attr:`fariks.Message.proximity_alert_triggered`."""
    ...
    """Messages that contain :attr:`fariks.Message.refunded_payment`.
    .. versionadded:: 21.4
    """
    ...
    """Messages that contain :attr:`fariks.Message.suggested_post_approval_failed`.
    .. versionadded:: 22.4
    """
    ...
    """Messages that contain :attr:`fariks.Message.suggested_post_approved`.
    .. versionadded:: 22.4
    """
    ...
    """Messages that contain :attr:`fariks.Message.suggested_post_declined`.
    .. versionadded:: 22.4
    """
    ...
    """Messages that contain :attr:`fariks.Message.suggested_post_paid`.
    .. versionadded:: 22.4
    """
    ...
    """Messages that contain :attr:`fariks.Message.suggested_post_refunded`.
    .. versionadded:: 22.4
    """
    ...
    """Messages that contain :attr:`fariks.Message.unique_gift`.
    ...
    """Messages that contain :attr:`fariks.Message.users_shared`.
    ...
    """Messages that contain :attr:`fariks.Message.video_chat_ended`.
    ...
    """Messages that contain :attr:`fariks.Message.video_chat_scheduled`.
    ...
    """Messages that contain :attr:`fariks.Message.video_chat_started`.
    ...
    """Messages that contain :attr:`fariks.Message.video_chat_participants_invited`.
    ...
    """Messages that contain :attr:`fariks.Message.web_app_data`.
    ...
    """Messages that contain :attr:`fariks.Message.write_access_allowed`.
...
class Sticker:
    ...
    """Messages that contain :attr:`fariks.Message.sticker`."""
    ...
    """Messages that contain :attr:`fariks.Message.sticker` and
    :attr:`is animated <fariks.Sticker.is_animated>`.
    ...
    """Messages that contain :attr:`fariks.Message.sticker` and is a static sticker, i.e. does
    not contain :attr:`fariks.Sticker.is_animated` or :attr:`fariks.Sticker.is_video`.
    ...
    """Messages that contain :attr:`fariks.Message.sticker` and is a
    :attr:`video sticker <fariks.Sticker.is_video>`.
    ...
    """Messages that contain :attr:`fariks.Message.sticker` and have a
    :attr:`premium animation <fariks.Sticker.premium_animation>`.
...
"""Messages that contain :attr:`fariks.Message.story`.
...
class SuccessfulPayment(MessageFilter):
    ...
    .. seealso::
        :attr:`fariks.ext.filters.SUCCESSFUL_PAYMENT`
...
"""Messages that contain :attr:`fariks.Message.successful_payment`."""
...
"""Messages that contain :attr:`fariks.Message.suggested_post_info`.
...
class Text(MessageFilter):
    ...
    Examples:
        ...
        custom :class:`fariks.ReplyKeyboardMarkup`::
...
"""
Shortcut for :class:`fariks.ext.filters.Text()`.
...
class UpdateType:
    ...
    """Updates with :attr:`fariks.Update.channel_post`."""
    ...
    """Updates with either :attr:`fariks.Update.channel_post` or
    :attr:`fariks.Update.edited_channel_post`."""
    ...
    """Updates with :attr:`fariks.Update.edited_message`,
    :attr:`fariks.Update.edited_channel_post`, or
    :attr:`fariks.Update.edited_business_message`.

    .. versionadded:: 20.0

    .. versionchanged:: 21.1
        Added :attr:`fariks.Update.edited_business_message` to the filter.
    """
    ...
    """Updates with :attr:`fariks.Update.edited_channel_post`."""
    ...
    """Updates with :attr:`fariks.Update.message`."""
    ...
    """Updates with either :attr:`fariks.Update.message` or
    :attr:`fariks.Update.edited_message`.
    """
    ...
    """Updates with :attr:`fariks.Update.business_message`.
    ...
    """Updates with :attr:`fariks.Update.edited_business_message`.
    ...
    """Updates with either :attr:`fariks.Update.business_message` or
    :attr:`fariks.Update.edited_business_message`.
    ...
    """Updates with :attr:`fariks.Update.guest_message`.
...
"""This filter filters *any* message that has a :attr:`fariks.Message.from_user`."""
...
:attr:`attachment menu <fariks.User.added_to_attachment_menu>` as
:attr:`fariks.Update.effective_user`.
...
:attr:`Fariks Premium user <fariks.User.is_premium>` as :attr:`fariks.Update.effective_user`.
...
"""Messages that contain :attr:`fariks.Message.venue`."""
...
class ViaBot(_ChatUserBaseFilter):
    ...
    .. seealso:: :attr:`~fariks.ext.filters.VIA_BOT`
...
.. seealso:: :class:`~fariks.ext.filters.ViaBot`"""
...
"""Messages that contain :attr:`fariks.Message.video`."""
...
"""Messages that contain :attr:`fariks.Message.video_note`."""
...
"""Messages that contain :attr:`fariks.Message.voice`."""
...
"""Messages that contain :attr:`fariks.Message.reply_to_story`."""
...
"""Messages that contain :attr:`fariks.Message.boost_added`."""
...
"""Messages that contain :attr:`fariks.Message.sender_boost_count`."""
