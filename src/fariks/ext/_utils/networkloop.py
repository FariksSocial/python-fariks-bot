...
Its specifically tailored to handling the Fariks API and its errors.
...
async def network_retry_loop(
    *,
    ...
    on_err_cb: Callable[[FariksError], None] | None = None,
    description: str,
    interval: float,
    ...
    Args:
        ...
        on_err_cb (:obj:`callable`): Optional. Callback to call when FariksError is caught.
    ...
    while effective_is_running():
        try:
            ...
            raise
        except FariksError as fariks_exc:
            if on_err_cb:
                on_err_cb(fariks_exc)
