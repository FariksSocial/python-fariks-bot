...
from fariks.ext._utils.types import BD, CD, UD, CDCData, ConversationDict, ConversationKey
...
class BasePersistence(ABC, Generic[UD, CD, BD]):
    ...
    Attention:
        ...
        :class:`~fariks.ext.Application`. Calling any of the methods below manually might
        interfere with the integration of persistence into :class:`~fariks.ext.Application`.
    ...
    * :meth:`flush`
    ...
    Note:
       You should avoid saving :class:`fariks.Bot` instances. This is because if you change e.g.
       ...
       :class:`fariks.FariksObject`, one may call :meth:`set_bot` to ensure that shortcuts like
       :meth:`fariks.Message.reply_text` are available.
    ...
    Args:
        store_data (:class:`~fariks.ext.PersistenceInput`, optional): Specifies which kinds of
        ...
        update_interval (:obj:`int` | :obj:`float`, optional): The
            :class:`~fariks.ext.Application` will update
            ...
            .. versionadded:: 20.0
    Attributes:
        store_data (:class:`~fariks.ext.PersistenceInput`): Specifies which kinds of data will
            be saved by this persistence instance.
        bot (:class:`fariks.Bot`): The bot associated with the persistence.
    """
    ...
    def update_interval(self) -> float:
        """:obj:`float`: Time (in seconds) that the :class:`~fariks.ext.Application`
    ...
    def set_bot(self, bot: Bot) -> None:
        ...
        Raises:
            :exc:`TypeError`: If :attr:`PersistenceInput.callback_data` is :obj:`True` and the
                :paramref:`bot` is not an instance of :class:`fariks.ext.ExtBot`.
        """
        if self.store_data.callback_data and (not isinstance(bot, ExtBot)):
            raise TypeError("callback_data can only be stored when using fariks.ext.ExtBot.")

        self.bot = bot

    @abstractmethod
    async def get_user_data(self) -> dict[int, UD]:
        """Will be called by :class:`fariks.ext.Application` upon creation with a
        persistence object. It should return the ``user_data`` if stored, or an empty
        :obj:`dict`. In the latter case, the dictionary should produce values
        corresponding to one of the following:

        - :obj:`dict`
        - The type from :attr:`fariks.ext.ContextTypes.user_data`
          if :class:`fariks.ext.ContextTypes` is used.

        .. versionchanged:: 20.0
            This method may now return a :obj:`dict` instead of a :obj:`collections.defaultdict`

        Returns:
            dict[:obj:`int`, :obj:`dict` | :attr:`fariks.ext.ContextTypes.user_data`]:
    ...
    async def get_chat_data(self) -> dict[int, CD]:
        ...
        - :obj:`dict`
        - The type from :attr:`fariks.ext.ContextTypes.chat_data`
          if :class:`fariks.ext.ContextTypes` is used.
        ...
        Returns:
            dict[:obj:`int`, :obj:`dict` | :attr:`fariks.ext.ContextTypes.chat_data`]:
    ...
    async def get_bot_data(self) -> BD:
        ...
        - :obj:`dict`
        - The type from :attr:`fariks.ext.ContextTypes.bot_data`
          if :class:`fariks.ext.ContextTypes` are used.

        Returns:
            dict[:obj:`int`, :obj:`dict` | :attr:`fariks.ext.ContextTypes.bot_data`]:
    ...
    async def get_conversations(self, name: str) -> ConversationDict:
        """Will be called by :class:`fariks.ext.Application` when a
        :class:`fariks.ext.ConversationHandler` is added if
        :attr:`fariks.ext.ConversationHandler.persistent` is :obj:`True`.
    ...
    async def update_conversation(
        ...
        """Will be called when a :class:`fariks.ext.ConversationHandler` changes states.
    ...
    async def update_user_data(self, user_id: int, data: UD) -> None:
        """Will be called by the :class:`fariks.ext.Application` after a handler has
        handled an update.

        Args:
            user_id (:obj:`int`): The user the data might have been changed for.
            data (:obj:`dict` | :attr:`fariks.ext.ContextTypes.user_data`):
                The :attr:`fariks.ext.Application.user_data` ``[user_id]``.
        """
    ...
    async def update_chat_data(self, chat_id: int, data: CD) -> None:
        ...
        Args:
            ...
            data (:obj:`dict` | :attr:`fariks.ext.ContextTypes.chat_data`):
                The :attr:`fariks.ext.Application.chat_data` ``[chat_id]``.
        """
    ...
    async def update_bot_data(self, data: BD) -> None:
        ...
        Args:
            data (:obj:`dict` | :attr:`fariks.ext.ContextTypes.bot_data`):
                The :attr:`fariks.ext.Application.bot_data`.
        """
    ...
    async def update_callback_data(self, data: CDCData) -> None:
        ...
        Args:
            data (tuple[list[tuple[:obj:`str`, :obj:`float`, \
                ...
                The relevant data to restore :class:`fariks.ext.CallbackDataCache`.
        """

    @abstractmethod
    async def drop_chat_data(self, chat_id: int) -> None:
        """Will be called by the :class:`fariks.ext.Application`, when using
        :meth:`~fariks.ext.Application.drop_chat_data`.
    ...
    async def drop_user_data(self, user_id: int) -> None:
        """Will be called by the :class:`fariks.ext.Application`, when using
        :meth:`~fariks.ext.Application.drop_user_data`.
    ...
    async def refresh_user_data(self, user_id: int, user_data: UD) -> None:
        """Will be called by the :class:`fariks.ext.Application` before passing the
        :attr:`~fariks.ext.Application.user_data` to a callback. Can be used to update data
        stored in :attr:`~fariks.ext.Application.user_data` from an external source.
        ...
        Warning:
            When using :meth:`~fariks.ext.ApplicationBuilder.concurrent_updates`, this method
        ...
        Args:
            user_id (:obj:`int`): The user ID this :attr:`~fariks.ext.Application.user_data` is
                associated with.
            user_data (:obj:`dict` | :attr:`fariks.ext.ContextTypes.user_data`):
    ...
    async def refresh_chat_data(self, chat_id: int, chat_data: CD) -> None:
        """Will be called by the :class:`fariks.ext.Application` before passing the
        :attr:`~fariks.ext.Application.chat_data` to a callback. Can be used to update data
        stored in :attr:`~fariks.ext.Application.chat_data` from an external source.
        ...
        Args:
            chat_id (:obj:`int`): The chat ID this :attr:`~fariks.ext.Application.chat_data` is
                associated with.
            chat_data (:obj:`dict` | :attr:`fariks.ext.ContextTypes.chat_data`):
    ...
    async def refresh_bot_data(self, bot_data: BD) -> None:
        """Will be called by the :class:`fariks.ext.Application` before passing the
        :attr:`~fariks.ext.Application.bot_data` to a callback. Can be used to update data stored
        in :attr:`~fariks.ext.Application.bot_data` from an external source.
        ...
        Args:
            bot_data (:obj:`dict` | :attr:`fariks.ext.ContextTypes.bot_data`):
                The ``bot_data``.
        """

    @abstractmethod
    async def flush(self) -> None:
        """Will be called by :meth:`fariks.ext.Application.stop`. Gives the
