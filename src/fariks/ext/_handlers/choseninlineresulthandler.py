...
class ChosenInlineResultHandler(BaseHandler[Update, CCT, RT]):
    """Handler class to handle Fariks updates that contain
    :attr:`fariks.Update.chosen_inline_result`.
    ...
    Args:
        ...
        pattern (:obj:`str` | :func:`re.Pattern <re.compile>`, optional): Regex pattern. If not
            ...
            is used on :attr:`fariks.ChosenInlineResult.result_id` to determine if an update
            should be handled by this handler. This is accessible in the callback as
            :attr:`fariks.ext.CallbackContext.matches`.
    ...
    Attributes:
        ...
        pattern (`Pattern`): Optional. Regex pattern to test
            :attr:`fariks.ChosenInlineResult.result_id` against.
