...
class CallbackQueryHandler(BaseHandler[Update, CCT, RT]):
    """Handler class to handle Fariks
    :attr:`callback queries <fariks.Update.callback_query>`. Optionally based on a regex.

    Read the documentation of the :mod:`re` module for more information.

    Note:
        * If your bot allows arbitrary objects as
          :paramref:`~fariks.InlineKeyboardButton.callback_data`, it may happen that the
          original :attr:`~fariks.InlineKeyboardButton.callback_data` for the incoming
          :class:`fariks.CallbackQuery` can not be found. This is the case when either a
          malicious client tempered with the :attr:`fariks.CallbackQuery.data` or the data was
          simply dropped from cache or not persisted. In these
          cases, an instance of :class:`fariks.ext.InvalidCallbackData` will be set as
          :attr:`fariks.CallbackQuery.data`.
        ...
        * If neither :paramref:`pattern` nor :paramref:`game_pattern` is set, `any`
          ...
          :attr:`~fariks.CallbackQuery.game_short_name` will `not` be considered and vice versa.
          If both patterns are set, queries with either :attr:
          `~fariks.CallbackQuery.game_short_name` or :attr:`~fariks.CallbackQuery.data`
    ...
    Args:
        ...
        pattern (:obj:`str` | :func:`re.Pattern <re.compile>` | :obj:`callable` | :obj:`type`, \
            optional):
            Pattern to test :attr:`fariks.CallbackQuery.data` against. If a string or a regex
            pattern is passed, :func:`re.match` is used on :attr:`fariks.CallbackQuery.data` to
            determine if an update should be handled by this handler. If your bot allows arbitrary
            objects as :paramref:`~fariks.InlineKeyboardButton.callback_data`, non-strings will
            be accepted. To filter arbitrary objects you may pass:

            - a callable, accepting exactly one argument, namely the
              :attr:`fariks.CallbackQuery.data`. It must return :obj:`True` or
              :obj:`False`/:obj:`None` to indicate, whether the update should be handled.
            - a :obj:`type`. If :attr:`fariks.CallbackQuery.data` is an instance of that type
              (or a subclass), the update will be handled.

            If :attr:`fariks.CallbackQuery.data` is :obj:`None`, the
            :class:`fariks.CallbackQuery` update will not be handled.
        ...
        game_pattern (:obj:`str` | :func:`re.Pattern <re.compile>` | optional)
            Pattern to test :attr:`fariks.CallbackQuery.game_short_name` against. If a string or
            a regex pattern is passed, :func:`re.match` is used on
            :attr:`fariks.CallbackQuery.game_short_name` to determine if an update should be
    ...
    Attributes:
        ...
        pattern (:func:`re.Pattern <re.compile>` | :obj:`callable` | :obj:`type`): Optional.
            Regex pattern, callback or type to test :attr:`fariks.CallbackQuery.data` against.

            .. versionchanged:: 13.6
               Added support for arbitrary callback data.
        game_pattern (:func:`re.Pattern <re.compile>`): Optional.
            Regex pattern to test :attr:`fariks.CallbackQuery.game_short_name`
