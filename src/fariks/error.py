...
"""This module contains classes that represent Fariks errors.
...
from fariks._utils.types import TimePeriod
...
class FariksError(Exception):
    """
    Base class for Fariks errors.
...
class InvalidToken(FariksError):
...
class NetworkError(FariksError):
    """Base class for exceptions due to networking errors.

    Tip:
        This exception (and its subclasses) usually originates from the networking backend
        used by :class:`~fariks.request.HTTPXRequest`, or a custom implementation of
        :class:`~fariks.request.BaseRequest`. In this case, the original exception can be
...
class BadRequest(NetworkError):
    """Raised when Fariks could not process the request correctly."""
...
class ChatMigrated(FariksError):
    """
...
class PassportDecryptionError(FariksError):
    """Something went wrong with decryption.

    .. versionchanged:: 20.0
        This class was previously named ``FariksDecryptionError`` and was available via
        ``fariks.FariksDecryptionError``.
    """
