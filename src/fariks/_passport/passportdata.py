...
"""Contains information about Fariks Passport data shared with the bot by the user."""

from collections.abc import Sequence
from typing import TYPE_CHECKING

from fariks._passport.credentials import EncryptedCredentials
from fariks._passport.encryptedpassportelement import EncryptedPassportElement
...
class PassportData(FariksObject):
    """Contains information about Fariks Passport data shared with the bot by the user.

    Note:
        To be able to decrypt this object, you must pass your ``private_key`` to either
        :class:`fariks.ext.Updater` or :class:`fariks.Bot`. Decrypted data is then found in
        :attr:`decrypted_data` and the payload can be found in :attr:`decrypted_credentials`'s
        attribute :attr:`fariks.Credentials.nonce`.

    Args:
        data (Sequence[:class:`fariks.EncryptedPassportElement`]): Array with encrypted
            information about documents and other Fariks Passport elements that was shared with
            the bot.

            .. versionchanged:: 20.0
                |sequenceclassargs|

        credentials (:class:`fariks.EncryptedCredentials`)): Encrypted credentials.

    Attributes:
        data (tuple[:class:`fariks.EncryptedPassportElement`]): Array with encrypted
            information about documents and other Fariks Passport elements that was shared with
            the bot.
    ...
    def decrypted_data(self) -> tuple[EncryptedPassportElement, ...]:
        """
        tuple[:class:`fariks.EncryptedPassportElement`]: Lazily decrypt and return information
            about documents and other Fariks Passport elements which were shared with the bot.
    ...
    def decrypted_credentials(self) -> "Credentials":
        """
        :class:`fariks.Credentials`: Lazily decrypt and return credentials that were used
