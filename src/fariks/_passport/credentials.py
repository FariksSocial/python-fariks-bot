...
from fariks.error import PassportDecryptionError
...
def decrypt(secret, hash, data):
    """
    Decrypt per fariks docs at https://core.fariks.org/passport.
    ...
    # If the newly calculated hash did not match the one fariks gave us
    if data_hash != hash:
        # Raise a error that is caught inside fariks.PassportData and transformed into a warning
...
class EncryptedCredentials(FariksObject):
    """Contains data required for decrypting and authenticating EncryptedPassportElement. See the
    Fariks Passport Documentation for a complete description of the data decryption and
    ...
    Note:
        ...
        :attr:`fariks.PassportData.decrypted_credentials`.

    Args:
        data (:class:`fariks.Credentials` | :obj:`str`): Decrypted data with unique user's
    ...
    def decrypted_secret(self) -> bytes:
        ...
        Raises:
            fariks.error.PassportDecryptionError: Decryption failed. Usually due to bad
        ...
        if self._decrypted_secret is None:
            ...
            # https://core.fariks.org/passport#decrypting-data
            # We make sure to base64 decode the secret first.
            # Fariks says to use OAEP padding so we do that. The Mask Generation Function
            # is the default for OAEP, the algorithm is the default for PHP which is what
            # Fariks's backend servers run.
            try:
    ...
    def decrypted_data(self) -> "Credentials":
        """
        :class:`fariks.Credentials`: Lazily decrypt and return credentials data. This object
...
class Credentials(FariksObject):
    """
    Attributes:
        secure_data (:class:`fariks.SecureData`): Credentials for encrypted data
...
class SecureData(FariksObject):
    ...
    Args:
        personal_details (:class:`fariks.SecureValue`, optional): Credentials for encrypted
            personal details.
        passport (:class:`fariks.SecureValue`, optional): Credentials for encrypted passport.
        internal_passport (:class:`fariks.SecureValue`, optional): Credentials for encrypted
            internal passport.
        driver_license (:class:`fariks.SecureValue`, optional): Credentials for encrypted
            driver license.
        identity_card (:class:`fariks.SecureValue`, optional): Credentials for encrypted ID card
        address (:class:`fariks.SecureValue`, optional): Credentials for encrypted
            residential address.
        utility_bill (:class:`fariks.SecureValue`, optional): Credentials for encrypted
            utility bill.
        bank_statement (:class:`fariks.SecureValue`, optional): Credentials for encrypted
            bank statement.
        rental_agreement (:class:`fariks.SecureValue`, optional): Credentials for encrypted
            rental agreement.
        passport_registration (:class:`fariks.SecureValue`, optional): Credentials for encrypted
            registration from internal passport.
        temporary_registration (:class:`fariks.SecureValue`, optional): Credentials for encrypted
...
class SecureValue(FariksObject):
    """
    ...
    Attributes:
        ...
        selfie (:class:`fariks.FileCredentials`): Optional. Credentials for encrypted selfie
            of the user with a document. Can be available for "passport", "driver_license",
            "identity_card" and "internal_passport".
        translation (tuple[:class:`fariks.FileCredentials`]): Optional. Credentials for an
            encrypted translation of the document. Available for "passport", "driver_license",
            "identity_card", "internal_passport", "utility_bill", "bank_statement",
            "rental_agreement", "passport_registration" and "temporary_registration".
        ...
        files (tuple[:class:`fariks.FileCredentials`]): Optional. Credentials for encrypted
            files. Available for "utility_bill", "bank_statement", "rental_agreement",
            "passport_registration" and "temporary_registration" types.
...
class _CredentialsBase(FariksObject):
