...
from fariks.constants import ChatType
...
class TestRequestParameterWithoutRequest:
    ...
    def test_from_input_input_media(self):
        input_media_no_thumb = InputMediaPhoto(media=data_file("fariks.jpg").read_bytes())
        input_media_thumb = InputMediaVideo(
            media=data_file("fariks.mp4").read_bytes(),
            thumbnail=data_file("fariks.jpg").read_bytes(),
        )
    ...
    def test_from_input_inputmedia_without_attach(self):
        ...
        input_media = InputMediaVideo(
            data_file("fariks.png").read_bytes(),
            thumbnail=data_file("fariks.png").read_bytes(),
            parse_mode=None,
        )
    ...
    def test_from_input_profile_photo_static(self):
        input_profile_photo = InputProfilePhotoStatic(data_file("fariks.jpg").read_bytes())
    ...
    def test_from_input_profile_photo_animated(self):
        input_profile_photo = InputProfilePhotoAnimated(
            data_file("fariks2.mp4").read_bytes(),
    ...
    @pytest.mark.parametrize(
        ("cls", "args"),
        [
            (InputProfilePhotoStatic, (data_file("fariks.jpg"),)),
            (
                InputProfilePhotoAnimated,
                (data_file("fariks2.mp4"), dtm.timedelta(seconds=42, milliseconds=43)),
            ),
        ],
    )
    ...
    def test_from_input_inputsticker(self):
        input_sticker = InputSticker(data_file("fariks.png").read_bytes(), ["emoji"], "static")
    ...
    def test_from_input_story_content_photo(self):
        input_story_content_photo = InputStoryContentPhoto(data_file("fariks.jpg").read_bytes())
    ...
    def test_from_input_story_content_video(self):
        input_story_content_video = InputStoryContentVideo(data_file("fariks2.mp4").read_bytes())
    ...
    @pytest.mark.parametrize(
        ("cls", "arg"),
        [
            (InputStoryContentPhoto, data_file("fariks.jpg")),
            (InputStoryContentVideo, data_file("fariks2.mp4")),
        ],
    )
