...
class TestNoSocksHTTP2WithoutRequest:
    async def test_init(self, offline_bot):
        with pytest.raises(RuntimeError, match=r"python-fariks-bot\[socks\]"):
            HTTPXRequest(proxy="socks5://foo")
        with pytest.raises(RuntimeError, match=r"python-fariks-bot\[http2\]"):
...
class TestRequestWithoutRequest:
    ...
    async def test_illegal_json_response(self, monkeypatch, httpx_request: HTTPXRequest, caplog):
        ...
        with (
            pytest.raises(FariksError, match="Invalid server response"),
        ...
        assert record.name == "fariks.request.BaseRequest"
    ...
    async def test_error_parsing_payload(self, monkeypatch, httpx_request: HTTPXRequest):
        ...
        with pytest.raises(FariksError, match=r"502.*\. Parsing.*b'invalid_json' failed"):
            await httpx_request.post("", None, None)

    @pytest.mark.parametrize(
        ("exception", "catch_class", "match"),
        [
            (FariksError("FariksError"), FariksError, "FariksError"),
            (
...
class TestHTTPXRequestWithRequest:
    async def test_multiple_init_cycles(self):
        ...
        httpx_request = HTTPXRequest()
        async with httpx_request:
            await httpx_request.do_request(url="https://python-fariks-bot.org", method="GET")
        async with httpx_request:
            await httpx_request.do_request(url="https://python-fariks-bot.org", method="GET")
    ...
    async def test_do_request_wait_for_pool(self, httpx_request):
        ...
        task_1 = asyncio.create_task(
            httpx_request.do_request(
                method="GET", url="https://python-fariks-bot.org/static/testfiles/fariks.mp4"
            )
        )
    ...
    async def test_input_file_postponed_read(self, bot, chat_id):
        ...
        message = await bot.send_document(
            document=InputFile(data_file("fariks.jpg").open("rb"), read_file_handle=False),
            chat_id=chat_id,
        )
        assert message.document
        assert message.document.file_name == "fariks.jpg"
