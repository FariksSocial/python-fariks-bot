...
from fariks._files.inputstorycontent import InputStoryContentVideo
from fariks._files.sticker import Sticker
from fariks._gifts import AcceptedGiftTypes, Gift
...
from fariks._inputchecklist import InputChecklist, InputChecklistTask
from fariks._message import Message
from fariks._ownedgift import OwnedGiftRegular, OwnedGifts
...
from fariks.constants import InputProfilePhotoType, InputStoryContentType
...
class TestBusinessMethodsWithoutRequest(BusinessMethodsTestBase):
    ...
    async def test_set_business_account_profile_photo(self, offline_bot, monkeypatch, is_public):
        ...
        kwargs = {
            ...
            "photo": InputProfilePhotoStatic(
                photo=data_file("fariks.jpg").read_bytes(),
            ),
        }
    ...
    async def test_set_business_account_profile_photo_local_file(self, offline_bot, monkeypatch):
        async def make_assertion(*args, **kwargs):
            ...
            assert (photo_dict := params.get("photo")).get("type") == InputProfilePhotoType.STATIC
            assert photo_dict["photo"] == data_file("fariks.jpg").as_uri()
    ...
    async def test_post_story_all_args(self, offline_bot, monkeypatch, active_period):
        content = InputStoryContentPhoto(photo=data_file("fariks.jpg").read_bytes())
    ...
    async def test_post_story_local_file(self, offline_bot, monkeypatch, active_period):
        ...
        async def make_assertion(*args, **kwargs):
            ...
            assert content_dict["photo"] == data_file("fariks.jpg").as_uri()
            assert not request_data.multipart_data
