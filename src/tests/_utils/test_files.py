...
import fariks._utils.datetime
import fariks._utils.files
from fariks import Animation, InputFile, MessageEntity
...
class TestFiles:
    ...
    def test_is_local_file(self, string, expected):
        assert fariks._utils.files.is_local_file(string) == expected
    ...
    def test_parse_file_input_string(self, string, expected_local, expected_non_local):
        assert fariks._utils.files.parse_file_input(string, local_mode=True) == expected_local

        if expected_non_local is InputFile:
            assert isinstance(
                fariks._utils.files.parse_file_input(string, local_mode=False), InputFile
            )
        elif expected_non_local is ValueError:
            with pytest.raises(ValueError, match="but local mode is not enabled\\."):
                fariks._utils.files.parse_file_input(string, local_mode=False)
        else:
            assert (
                fariks._utils.files.parse_file_input(string, local_mode=False)
                == expected_non_local
    ...
    def test_parse_file_input_file_like(self):
        ...
        with source_file.open("rb") as file:
            parsed = fariks._utils.files.parse_file_input(file)

        assert isinstance(parsed, InputFile)
        assert parsed.filename == "game.gif"

        with source_file.open("rb") as file:
            parsed = fariks._utils.files.parse_file_input(file, filename="test_file")
    ...
    def test_parse_file_input_bytes(self):
        ...
        parsed = fariks._utils.files.parse_file_input(source_file.read_bytes())

        assert isinstance(parsed, InputFile)
        assert parsed.filename == "application.octet-stream"

        parsed = fariks._utils.files.parse_file_input(
    ...
    def test_parse_file_input_tg_object(self):
        ...
        assert fariks._utils.files.parse_file_input(animation, Animation) == "file_id"
        assert fariks._utils.files.parse_file_input(animation, MessageEntity) is animation

    @pytest.mark.parametrize("obj", [{1: 2}, [1, 2], (1, 2)])
    def test_parse_file_input_other(self, obj):
        assert fariks._utils.files.parse_file_input(obj) is obj

    @pytest.mark.parametrize("attach", [True, False])
    def test_parse_file_input_attach(self, attach):
        source_file = data_file("text_file.txt")
        parsed = fariks._utils.files.parse_file_input(source_file.read_bytes(), attach=attach)

        assert isinstance(parsed, InputFile)
        assert bool(parsed.attach_name) is attach

    def test_load_file_none(self):
        assert fariks._utils.files.load_file(None) == (None, None)

    @pytest.mark.parametrize("arg", [b"bytes", "string", InputFile(b"content"), Path("file/path")])
    def test_load_file_no_file(self, arg):
        out = fariks._utils.files.load_file(arg)
        assert out[0] is None
        assert out[1] is arg

    def test_load_file_file_handle(self):
        out = fariks._utils.files.load_file(data_file("fariks.gif").open("rb"))
        assert out[0] == "fariks.gif"
        assert out[1] == data_file("fariks.gif").read_bytes()

    def test_load_file_subprocess_pipe(self):
        png_file = data_file("fariks.png")
        ...
        out = fariks._utils.files.load_file(proc.stdout)
    ...
    def test_parse_file_input_path_no_resource_warning(self):
        ...
        test_file = data_file(filename="fariks.png")

        # This should not raise a ResourceWarning
        result = fariks._utils.files.parse_file_input(test_file)
