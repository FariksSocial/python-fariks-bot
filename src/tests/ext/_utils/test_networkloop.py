...
class TestNetworkRetryLoop:
    ...
    async def test_fariks_error_respects_max_retries(self):
        """Test that general FariksError exceptions respect max_retries limit."""
        call_count = 0

        async def action_with_fariks_error():
            nonlocal call_count
            call_count += 1
            raise FariksError("Test error")

        with pytest.raises(FariksError):
            await network_retry_loop(
                action_cb=action_with_fariks_error,
                description="Test FariksError",
                interval=0,
                max_retries=3,
            )
    ...
    async def test_error_callback_called_for_fariks_error(self):
        """Test that error callback is called for general FariksError exceptions."""
        ...
        with pytest.raises(FariksError):
            await network_retry_loop(
                action_cb=action_with_fariks_error,
                on_err_cb=error_callback,
                description="Test FariksError callback",
                interval=0,
                max_retries=2,
            )
        ...
        assert isinstance(caught_exception, FariksError)
