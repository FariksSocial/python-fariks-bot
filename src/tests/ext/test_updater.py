...
from fariks import Bot, InlineKeyboardButton, InlineKeyboardMarkup, Update
from fariks.error import InvalidToken, RetryAfter, FariksError, TimedOut
from fariks.ext import ExtBot, InvalidCallbackData, Updater
...
if TEST_WITH_OPT_DEPS:
    ...
    from fariks.ext._utils.webhookhandler import WebhookServer
...
class TestNoWebhooks:
    async def test_no_webhooks(self, bot):
        async with Updater(bot=bot, update_queue=asyncio.Queue()) as updater:
            with pytest.raises(RuntimeError, match=r"python-fariks-bot\[webhooks\]"):
...
class TestUpdater:
    ...
    async def test_polling_mark_updates_as_read(self, monkeypatch, updater, caplog):
        ...
        for record in caplog.records:
            ...
            assert record.name == "fariks.ext.Updater"
    ...
    @pytest.mark.parametrize("exception_class", [InvalidToken, FariksError])
    ...
    async def test_start_polling_bootstrap_retries(
        ...
        async with updater:
            if exception_class == InvalidToken:
                ...
                with pytest.raises(FariksError, match=str(retries + 1)):
                    await updater.start_polling(bootstrap_retries=retries)

    @pytest.mark.parametrize(
        ("error", "callback_should_be_called"),
        argvalues=[
            (FariksError("TestMessage"), True),
            (RetryAfter(1), False),
            (TimedOut("TestMessage"), False),
        ],
        ids=("FariksError", "RetryAfter", "TimedOut"),
    )
                ...
                if callback_should_be_called:
                    ...
                    if custom_error_callback:
                        ...
                        assert any(
                            ...
                            and record.name == "fariks.ext.Updater"
                            for record in caplog.records
                        )
    ...
    async def test_webhook_arbitrary_callback_data(
        ...
        """Here we only test one simple setup. fariks.ext.ExtBot.insert_callback_data is tested
    ...
    async def test_webhook_invalid_ssl(self, monkeypatch, updater):
        ...
        port = randrange(1024, 49152)  # Select random port
        async with updater:
            with pytest.raises(FariksError, match="Invalid SSL"):
                await updater.start_webhook(
                    ip,
                    port,
                    url_path="TOKEN",
    ...
    async def test_webhook_ssl_just_for_fariks(self, monkeypatch, updater):
        """Here we just test that the SSL info is pased to Fariks, but __not__ to the
        webhook server"""
        ...
        monkeypatch.setattr(
            "fariks.ext._utils.webhookhandler.WebhookServer.__init__", webhook_server_init
        )
    ...
    async def test_webhook_update_de_json_fails(self, monkeypatch, updater, caplog):
        ...
        async with updater:
            ...
            assert caplog.records[-1].name == "fariks.ext.Updater"
