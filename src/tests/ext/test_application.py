...
from fariks import Bot, Chat, Message, MessageEntity, User
from fariks.error import InvalidToken, FariksError
from fariks.ext import (
    Application,
    ApplicationBuilder,
...
class TestApplication:
    ...
    def callback_raise_error(self, error_message: str):
        async def callback(update, context):
            raise FariksError(error_message)
    ...
    async def callback_context(self, update, context):
        if (
            ...
            and isinstance(context.error, FariksError)
        ):
            self.received = context.error.message
    ...
    def test_double_add_error_handler(self, app, caplog):
        ...
        with caplog.at_level(logging.DEBUG):
            ...
            assert caplog.records[-1].name == "fariks.ext.Application"
        ...
        with caplog.at_level(logging.ERROR):
            async with app:
                ...
                assert any(
                    ...
                    and record.name == "fariks.ext.Application"
    ...
    def test_run_polling_basic(self, app, monkeypatch, caplog):
        ...
        exception = FariksError("This is a test error")
        assertions = {}
    ...
    def test_raise_system_exit_keyboard_jobs_handlers(
        ...
        async def handler_callback(update, context):
            if kind == "handler":
                ...
                raise FariksError("Triggering error callback")
        ...
        for record in caplog.records:
            if record.getMessage() != "Dropping pending update: will_not_be_processed":
                continue
            assert record.name == "fariks.ext.Application"
    ...
    async def test_run_polling_webhook_infinite_bootstrap_retries(
        ...
        def thread_target():
            ...
            async def initialize(*args, **kwargs):
                ...
                self.count += 1
                if self.count == 1:
                    raise FariksError("Test Exception")
    ...
    def test_signal_handlers(self, offline_bot, monkeypatch):
        ...
        async def quick_initialize(*args, **kwargs):
            ...
            raise FariksError("Test completed successfully")

        monkeypatch.setattr(app, "initialize", quick_initialize)

        with pytest.raises(FariksError, match="Test completed successfully"):
            app.run_polling(close_loop=False)
