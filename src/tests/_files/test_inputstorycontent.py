...
from fariks import InputFile, InputStoryContent, InputStoryContentPhoto, InputStoryContentVideo
from fariks.constants import InputStoryContentType
...
class TestInputStoryContentPhotoWithoutRequest(InputStoryContentPhotoTestBase):
    ...
    def test_with_local_files(self):
        inst = InputStoryContentPhoto(photo=data_file("fariks.jpg"))
        assert inst.photo == data_file("fariks.jpg").as_uri()
...
class TestInputStoryContentVideoWithoutRequest(InputStoryContentVideoTestBase):
    ...
    def test_with_local_files(self):
        inst = InputStoryContentVideo(video=data_file("fariks.mp4"))
        assert inst.video == data_file("fariks.mp4").as_uri()
