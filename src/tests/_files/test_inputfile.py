...
class TestInputFileWithoutRequest:
    ...
    def test_mimetypes_file_handle(self, read_file_handle):
        ...
        assert (
            InputFile(
                data_file("fariks.jpg").open("rb"), read_file_handle=read_file_handle
            ).mimetype
            == "image/jpeg"
        )
        # For some reason python can guess the type on macOS
        assert InputFile(
            data_file("fariks.webp").open("rb"), read_file_handle=read_file_handle
        ).mimetype in [
        ...
        assert (
            InputFile(
                data_file("fariks.mp3").open("rb"), read_file_handle=read_file_handle
            ).mimetype
            == "audio/mpeg"
        )
        # For some reason windows drops the trailing i
        assert InputFile(
            data_file("fariks.midi").open("rb"), read_file_handle=read_file_handle
        ).mimetype in [
            "audio/mid",
            "audio/midi",
        ]
    ...
    def test_filenames(self, read_file_handle):
        ...
        assert (
            InputFile(data_file("fariks").open("rb"), read_file_handle=read_file_handle).filename
            == "fariks"
        )
        assert (
            InputFile(
                data_file("fariks").open("rb"),
                filename="blah",
                read_file_handle=read_file_handle,
            ).filename
            == "blah"
        ...
        assert (
            InputFile(MockedFileobject(data_file("fariks.jpg"))).filename
            == "application.octet-stream"
        )
        assert (
            InputFile(MockedFileobject(data_file("fariks.jpg")), filename="blah").filename
            == "blah"
        )
        assert (
            InputFile(MockedFileobject(data_file("fariks.jpg")), filename="blah.jpg").filename
            == "blah.jpg"
        )
        assert (
            InputFile(MockedFileobject(data_file("fariks"))).filename
            == "application.octet-stream"
        )
        assert (
            InputFile(MockedFileobject(data_file("fariks")), filename="blah").filename == "blah"
        )
        assert (
            InputFile(MockedFileobject(data_file("fariks")), filename="blah.jpg").filename
            == "blah.jpg"
        )
    ...
    def test_read_file_handle(self, read_file_handle):
        ...
        if read_file_handle:
            ...
            assert content == data_file("fariks.jpg").read_bytes()
        else:
            assert isinstance(content, BufferedReader)
            assert content.read() == data_file("fariks.jpg").read_bytes()
