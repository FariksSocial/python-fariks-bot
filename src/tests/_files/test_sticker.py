...
from fariks.constants import ParseMode, StickerFormat, StickerType
from fariks.error import BadRequest, FariksError
...
class StickerTestBase:
    # sticker_file_url = 'https://python-fariks-bot.org/static/testfiles/fariks.webp'
    # Serving sticker from gh since our server sends wrong content_type
    sticker_file_url = (
        "https://github.com/FariksSocial/python-fariks-bot/blob/master"
        "/tests/data/fariks.webp?raw=true"
    )
...
class TestStickerWithRequest(StickerTestBase):
    ...
    async def test_send_with_emoji(self, bot, chat_id):
        message = await bot.send_sticker(
            chat_id=chat_id, sticker=data_file("fariks.jpg"), emoji=self.emoji
        )
    ...
    async def test_error_send_empty_file(self, bot, chat_id):
        with Path(os.devnull).open("rb") as file, pytest.raises(FariksError):
            await bot.send_sticker(chat_id, file)

    async def test_error_send_empty_file_id(self, bot, chat_id):
        with pytest.raises(FariksError):
...
class TestStickerSetWithRequest:
    ...
    async def _create_sticker_set(
        ...
        try:
            ...
            s = await bot.create_new_sticker_set(
                ...
                stickers=[
                    InputSticker(
                        data_file("fariks.webp").open("rb"),
                        emoji_list=["😄"],
                        format=StickerFormat.STATIC,
                    ),
                    InputSticker(
                        data_file("fariks_animated_sticker.tgs").open("rb"),
                        emoji_list=["😄", "⭐"],
                        format=StickerFormat.ANIMATED,
                    ),
                    # works:
                    InputSticker(
                        data_file("fariks_video_sticker.webm").open("rb"),
                        emoji_list=["😄", "🔥"],
    ...
    async def test_add_sticker_to_set(
        ...
        with data_file("fariks_sticker.png").open("rb") as f:
        ...
        assert await bot.add_sticker_to_set(  # Also test with file input and mask
            ...
            sticker_set_name,
            sticker=InputSticker(
                sticker=data_file("fariks.webp").open("rb"),
                emoji_list=["😄"],
        ...
        assert await bot.add_sticker_to_set(
            ...
            sticker_set_name,
            sticker=InputSticker(
                sticker=data_file("fariks_animated_sticker.tgs").open("rb"),
                emoji_list=["😄"],
                format=StickerFormat.ANIMATED,
            ),
        )
        ...
        assert await bot.add_sticker_to_set(
            ...
            sticker_set_name,
            sticker=InputSticker(
                sticker=data_file("fariks_video_sticker.webm").open("rb"),
                emoji_list=["🤔"],
                format=StickerFormat.VIDEO,
            ),
        )
    ...
    @pytest.mark.skip(
        "Skipped for now since Fariks throws a 'File is too big' error "
