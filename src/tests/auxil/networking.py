...
from fariks._utils.types import ODVInput
from fariks.error import BadRequest, RetryAfter, TimedOut
...
class OfflineRequest(BaseRequest):
    """This Request class disallows making requests to Fariks's servers.
...
async def expect_bad_request(func, message, reason):
    """
    Wrapper for testing bot functions expected to result in an :class:`fariks.error.BadRequest`.
...
async def send_webhook_message(
    ...
    if secret_token:
        headers["X-Fariks-Bot-Api-Secret-Token"] = secret_token
