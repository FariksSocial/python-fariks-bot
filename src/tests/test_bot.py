...
from fariks._utils.datetime import UTC, from_timestamp, localize, to_timestamp
from fariks._utils.defaultvalue import DEFAULT_NONE
from fariks._utils.strings import to_camel_case
from fariks.constants import (
    ChatAction,
    InlineQueryLimit,
...
from fariks.error import BadRequest, EndPointNotFound, InvalidToken, TimedOut
from fariks.ext import ExtBot, InvalidCallbackData
from fariks.helpers import escape_markdown
from fariks.request import BaseRequest, HTTPXRequest, RequestData
...
async def media_message(bot, chat_id):
    # mostly used in tests for edit_message and hence can't be reused
    with data_file("fariks.ogg").open("rb") as f:
...
class TestBotWithoutRequest:
    ...
    @pytest.mark.parametrize(
        ("cls", "logger_name"), [(Bot, "fariks.Bot"), (ExtBot, "fariks.ext.ExtBot")]
    )
    async def test_bot_method_logging(self, offline_bot: PytestExtBot, cls, logger_name, caplog):
        ...
        with caplog.at_level(logging.DEBUG, logger="fariks"):
    ...
    async def test_get_updates_deserialization_error(self, offline_bot, monkeypatch, caplog):
        ...
        assert caplog.records[0].name == "fariks.ext.ExtBot"
    ...
    async def test_answer_inline_query(
        ...
        async def make_assertion(url, request_data: RequestData, *args, **kwargs):
            expected = {
                ...
                "results": [
                    ...
                    },
                    {
                        ...
                        "type": "document",
                        "document_url": (
                            "https://raw.githubusercontent.com/python-fariks-bot"
            ...
            if button_type == "start":
                ...
                button_dict = {
                    "text": "button_text",
                    "web_app": {"url": "https://python-fariks-bot.org"},
                }
        ...
        results = [
            ...
            InlineQueryResultDocument(
                id="123",
                document_url=(
                    "https://raw.githubusercontent.com/python-fariks-bot/logos/master/"
        ...
        if button_type == "start":
            ...
            button = InlineQueryResultsButton(
                text="button_text", web_app=WebAppInfo("https://python-fariks-bot.org")
            )
        else:
            button = None
                ...
                "results": [
                    ...
                    },
                    {
                        ...
                        "document_url": (
                            ...
                            "python-fariks-bot/logos/master/logo/png/"
                            "ptb-logo_240.png"
                        ),
                        "mime_type": "image/png",
                        "caption": "ptb_logo",
    ...
    async def test_timeout_propagation_explicit(self, monkeypatch, offline_bot, chat_id):
        ...
        with pytest.raises(OkException):
            await offline_bot.send_photo(
                chat_id, data_file("fariks.jpg").open("rb"), read_timeout=timeout
            )
    ...
    async def test_timeout_propagation_implicit(self, monkeypatch, offline_bot, chat_id):
        ...
        # Test file uploading
        with pytest.raises(OkException):
            await offline_bot.send_photo(chat_id, data_file("fariks.jpg").open("rb"))
    ...
    async def test_replace_callback_data_answer_inline_query(self, monkeypatch, cdc_bot, chat_id):
        ...
        try:
            ...
            no_replace_button = InlineKeyboardButton(
                text="no_replace", url="http://python-fariks-bot.org/"
            )
            ...
            results = [
                ...
                InlineQueryResultVoice(
                    "22",
                    "https://python-fariks-bot.org/static/testfiles/fariks.ogg",
                    title="second",
                ),
            ]
    ...
    async def test_set_message_reaction(self, offline_bot, monkeypatch):
        ...
        having to do multiple requests to Fariks"""
    ...
    @pytest.mark.filterwarnings("ignore::fariks.warnings.PTBUserWarning")
    async def test_do_api_request_media_write_timeout(self, offline_bot, chat_id, monkeypatch):
        ...
        await offline_bot.do_api_request(
            ...
            api_kwargs={
                ...
                "document": InputFile(data_file("fariks.png").open("rb")),
            },
        )
        assert test_flag == (
            DEFAULT_NONE,
...
class TestBotWithRequest:
    ...
    async def test_send_poll_media_parameters(self, bot, channel_id):
        with (
            data_file("fariks.jpg").open("rb") as photo_file,
    ...
    async def test_set_webhook_get_webhook_info_and_delete_webhook(self, bot, use_ip, file_input):
        url = "https://python-fariks-bot.org/test/webhook"
        # Get the ip address of the website - dynamically just in case it ever changes
        ip = socket.gethostbyname("python-fariks-bot.org")
        max_connections = 7
    ...
    async def test_get_chat(self, bot, super_group_id):
        ...
        assert cfi.title == f">>> fariks.Bot(test) @{bot.username}"
    ...
    async def test_send_game(self, bot, chat_id):
        ...
        assert (
            ...
            == "A no-op test game, for python-fariks-bot bot framework testing."
        )
    ...
    async def test_set_chat_photo(self, bot, channel_id):
        ...
        with data_file("fariks_test_channel.jpg").open("rb") as f:
            await expect_bad_request(
                func, "Type of file mismatch", "Fariks did not accept the file."
            )
    ...
    async def test_set_chat_title(self, bot, channel_id):
        assert await bot.set_chat_title(channel_id, ">>> fariks.Bot() - Tests")
    ...
    async def test_send_message_default_link_preview_options(self, default_bot, chat_id):
        ...
        github_url = "https://github.com/FariksSocial/python-fariks-bot"
        website = "https://python-fariks-bot.org/"
    ...
    async def test_edit_message_text_default_link_preview_options(self, default_bot, chat_id):
        ...
        github_url = "https://github.com/FariksSocial/python-fariks-bot"
        website = "https://python-fariks-bot.org/"
        fariks_url = "https://fariks.com"
        base_1, base_2, base_3, base_4 = await asyncio.gather(
            *(default_bot.send_message(chat_id, fariks_url) for _ in range(4))
        )
    ...
    async def test_get_set_chat_menu_button(self, bot, chat_id):
        ...
        my_menu = MenuButtonWebApp("click me!", WebAppInfo("https://fariks.com/"))
    ...
    async def test_do_api_request_invalid_token(self, bot):
        ...
        with pytest.raises(
            InvalidToken, match="token was rejected by Fariks or the endpoint 'getMe'"
        ):
            await Bot("invalid_token").do_api_request("get_me")

        # same test, but with a valid token bot and unknown endpoint
        with pytest.raises(
            InvalidToken, match="token was rejected by Fariks or the endpoint 'unknownEndpoint'"
        ):
    ...
    async def test_do_api_request_basic_and_files(self, bot, chat_id, return_type):
        ...
        out.seek(0)
        assert out.read() == data_file("fariks.png").open("rb").read()
        assert result.document.file_name == "fariks.png"
    ...
    async def test_my_profile_photo(self, bot):
        ...
        await bot.set_my_profile_photo(
            InputProfilePhotoStatic(photo=data_file("fariks.jpg").read_bytes())
        )
        bot_profile_photos = await bot.get_user_profile_photos(bot.id)
