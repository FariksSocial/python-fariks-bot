...
class FariksParameter:
    """Represents the scraped Fariks parameter. Contains all relevant attributes needed for
    comparison. Relevant for both FariksMethod and FariksClass."""
...
@dataclass(slots=True, frozen=True)
class FariksClass:
    """Represents the scraped Fariks class. Contains all relevant attributes needed for
    comparison."""

    class_name: str
    class_parameters: list[FariksParameter]
    # class_description: str


@dataclass(slots=True, frozen=True)
class FariksMethod:
    """Represents the scraped Fariks method. Contains all relevant attributes needed for
    comparison."""

    method_name: str
    method_parameters: list[FariksParameter]
...
class Scraper:
    ...
    async def make_request(self) -> None:
        async with httpx.AsyncClient() as client:
            self.request = await client.get("https://core.fariks.com/bots/api", timeout=10)
        self.soup = BeautifulSoup(self.request.text, "html.parser")

    @overload
    def parse_docs(
        self, doc_type: Literal["method"]
    ) -> tuple[list[FariksMethod], list[str]]: ...

    @overload
    def parse_docs(self, doc_type: Literal["class"]) -> tuple[list[FariksClass], list[str]]: ...
        ...
        for unparsed in self.soup.select("h4 > a.anchor"):
            if "-" not in unparsed["name"]:
                ...
                if doc_type == "method" and name[0].lower() == name[0]:
                    ...
                    obj = FariksMethod(method_name=name, method_parameters=params)
                    argvalues.append(obj)
                    names.append(name)
                elif doc_type == "class" and is_pascal_case(name) and name not in IGNORED_OBJECTS:
                    params = parse_table_for_params(h4)
                    obj = FariksClass(class_name=name, class_parameters=params)
                    argvalues.append(obj)
                    names.append(name)

        return argvalues, names

    def collect_methods(self) -> tuple[list[FariksMethod], list[str]]:
        return self.parse_docs("method")

    def collect_classes(self) -> tuple[list[FariksClass], list[str]]:
        return self.parse_docs("class")


def parse_table_for_params(h4: Tag) -> list[FariksParameter]:
    """Parses the Fariks doc table and outputs a list of FariksParameter objects."""
    ...
    for tr in table.find_all("tr")[1:]:
        ...
        params.append(FariksParameter(param_name, param_type, param_required, param_desc))
