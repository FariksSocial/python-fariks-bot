...
from fariks.ext import InvalidCallbackData
from fariks.warnings import PTBDeprecationWarning
from tests.auxil.slots import mro_slots


class TestErrors:
    def test_fariks_error(self):
        with pytest.raises(FariksError, match=r"^test message$"):
            raise FariksError("test message")
        with pytest.raises(FariksError, match=r"^Test message$"):
            raise FariksError("Error: test message")
        with pytest.raises(FariksError, match=r"^Test message$"):
            raise FariksError("[Error]: test message")
        with pytest.raises(FariksError, match=r"^Test message$"):
            raise FariksError("Bad Request: test message")
    ...
    @pytest.mark.parametrize(
        ...
        [
            (FariksError("test message"), ["message"]),
