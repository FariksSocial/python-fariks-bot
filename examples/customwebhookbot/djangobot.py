...
async def fariks(request: HttpRequest) -> HttpResponse:
    """Handle incoming Fariks updates by putting them into the `update_queue`"""
...
urlpatterns = [
    path("fariks", fariks, name="Fariks updates"),
...
async def main() -> None:
    ...
    # Pass webhook settings to fariks
    await ptb_application.bot.set_webhook(url=f"{URL}/fariks", allowed_updates=Update.ALL_TYPES)
